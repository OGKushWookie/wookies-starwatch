(() => {
  ...
  const xpActiveTrend = (history, activity, windowName) => {
    const rawSamples = (Array.isArray(history) ? history : []).filter((sample) => sample.activities?.[activity]).sort((a, b) => a.at - b.at);
    ...
    const medianRate = (rows) => {
      const sorted = rows.map((pair) => pair.rate).sort((a, b) => a - b);
        ...
        <div class="so-xp-leaderboard-list">
          ${board.ranked.length ? board.ranked.map((row) => {
            const evidence = row.rateSource === "active"
              ? { ...row.trend, rate: row.trend.activeRate, elapsed: row.trend.activeElapsed, sampleCount: row.trend.activeSampleCount, outliers: row.trend.activeOutliers, withheld: row.trend.activeWithheld }
              : row.trend;
  ...
  const renderXpRankings = () => {
    ...
    return `
      ...
      <div class="so-xp-leaderboards">
        <p class="so-note">Rankings include you, squad members, and favorited profiles in one combined board per activity. Each player uses their reliable Active Sessions rate when available and automatically falls back to Wall Clock otherwise. The rate label on every row shows which source is being used. RSS remains a wall-clock rate from cumulative profile totals.</p>
      ...
      <p class="so-note">Active XP needs at least three accepted gain intervals spanning 10 minutes. Its robust median estimates the normal earning cycle without letting unrelated activity snapshots shorten that cycle. Until then, an eligible wall-clock average is used; players without either rate remain calibrating. RSS requires at least 60 minutes. Ties share the same place.</p>`;
  };
