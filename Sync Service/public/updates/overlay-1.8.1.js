  ...
  const renderXpLeaderboard = (keys, activity) => {
    ...
    return `
      <section class="so-xp-leaderboard-card">
        ...
        <div class="so-xp-leaderboard-foot">${board.ranked.length} ranked • ${board.calibrating.filter((row) => !row.trend.withheld).length} calibrating • ${board.calibrating.filter((row) => row.trend.withheld).length} withheld for outliers • ${state.xpRateMode === "active" ? "median continuous gain interval" : "wall-clock average"}</div>
      </section>`;
  };
  ...
  const renderXpRankings = () => {
    ...
    return `
      ...
      <div class="so-xp-leaderboards">
        <p class="so-note">Rankings include you, squad members, and favorited profiles. Active Sessions uses the median of continuous, positive per-activity XP intervals no more than 10 minutes apart; unchanged snapshots from other activities are ignored. Wall Clock includes idle/offline time. RSS remains a wall-clock rate from cumulative profile totals.</p>
      ...
      <p class="so-note">Active XP needs at least three accepted gain intervals spanning 10 minutes. Its robust median estimates the normal earning cycle without letting unrelated activity snapshots shorten that cycle. Players with sparse manual profile visits may have only a wall-clock rate until more continuous observations are contributed. RSS requires at least 60 minutes. Ties share the same place.</p>`;
  };
