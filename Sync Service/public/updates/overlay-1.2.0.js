(() => {
  ...
  const state = {
    ...
    sharedSummaries: {
      ...
      resources: saved.sharedSummaries?.resources && typeof saved.sharedSummaries.resources === "object" ? saved.sharedSummaries.resources : {},
    },
    resourceRankings: {
    ...
    sync: {
      ...
      summaryPulledAt: Number(saved.sync?.summaryPulledAt || 0),
      summaryPlayerSet: String(saved.sync?.summaryPlayerSet || ""),
      historyCursors: Number(saved.sync?.historySyncVersion || 0) >= HISTORY_SYNC_VERSION && saved.sync?.historyCursors && typeof saved.sync.historyCursors === "object" ? saved.sync.historyCursors : {},
  ...
  const queueSyncResource = (username, sample) => {
    if (!state.sync.enabled || !sample) return;
    state.sync.outboxResources = coalesceSyncSamples([...state.sync.outboxResources, { username: syncUsername(username), ...plain(sample) }]);
    scheduleSharedSync();
  };
