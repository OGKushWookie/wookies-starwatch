  ...
  const renderNodes = () => {
    ...
    return `
      ...
      <section><h3>Perfect-node finder</h3>
        ...
        <p class="so-note">${state.officialApi.jumps ? `Journal: ${compactNumber(state.officialApi.jumps)} jumps across ${compactNumber(state.officialApi.journalSystems)} coordinates. ` : ""}Every exact native <strong>nodeQuality = 100</strong> observation is saved automatically and follows the linked overlay account. Legacy unverified records stay hidden until rescanned.</p>
      </section>
      <section><h3>Perfect-node matches</h3><div class="so-list">
