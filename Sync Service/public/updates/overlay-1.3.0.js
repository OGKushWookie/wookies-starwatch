(() => {
  ...
  const TAB_META = {
    ...
    profiles: ["Player Intel", "Profiles, gear and build comparisons"],
  ...
  const state = {
    ...
    session: saved.session && typeof saved.session === "object" ? saved.session : {
      ...
      baselinePerfect: Object.keys(saved.systems || {}).length,
    },
    marketTools: {
  ...
  const eventSystem = (row) => row?.system && typeof row.system === "object" ? row.system : row || {};
  const normalizeEventRows = (kind, rows, fetchedAt) => (Array.isArray(rows) ? rows : []).slice(0, 200).map((row, index) => {
    const system = eventSystem(row);
    const startsAt = apiTimeMs(row?.spawnAt ?? row?.createdAt);
    const endsAt = apiTimeMs(row?.expires_at ?? row?.closesAt ?? row?.resetsAt);
    const name = kind === "rune" ? String(row?.rune_name || "Active rune")
      : kind === "solo" ? String(row?.name || system?.name || "Solo dungeon")
        : String(row?.name || "Dungeon");
    const x = finite(system?.coordinate_x ?? row?.coordinate_x);
    const y = finite(system?.coordinate_y ?? row?.coordinate_y);
    const z = finite(system?.coordinate_z ?? row?.coordinate_z ?? 1) ?? 1;
    ...
    return {
      ...
      status: String(row?.status || row?.type || "").slice(0, 80),
      ...
      weakness: String(row?.boss_weakness || "").slice(0, 80),
  ...
  const applyOfficialEvents = (route, result) => {
    ...
    const sourceRows = route === "activerunes"
      ? (Array.isArray(payload.runeSystems) ? payload.runeSystems : payload)
      : (Array.isArray(payload.dungeons) ? payload.dungeons : payload);
    const rows = normalizeEventRows(kind, sourceRows, result?.fetchedAt);
  ...
  const runSharedSync = async (force = false) => {
    ...
    const hasOutbound = state.sync.outboxXp.length > 0 || state.sync.outboxResources.length > 0
      || Object.keys(state.sync.outboxProfiles).length > 0 || state.sync.outboxSystems.length > 0;
    ...
    try {
      ...
      continueSync = state.sync.outboxXp.length > 0 || state.sync.outboxResources.length > 0
        || Object.keys(state.sync.outboxProfiles).length > 0 || state.sync.outboxSystems.length > 0 || result.systemsMore === true;
    } catch (error) {
  ...
  const updateSessionFromProfile = (profile) => {
    ...
    if (!state.session.startAt || now - Number(state.session.lastActiveAt || now) > 6 * 60 * 60 * 1000) {
      state.session = {
        ...
        baselinePerfect: Object.keys(state.systems).length,
      };
    }
  ...
  ].filter((event) => !event.endsAt || event.endsAt > Date.now() - 5 * 60 * 1000)
  ...
  const renderNodes = () => {
    ...
    return `
      ...
      <section><h3>Galaxy event radar</h3>
        <p class="so-note">Official dungeon, solo-dungeon and active-rune feeds. Colored map markers are informational and never initiate travel.</p>
        <div class="so-event-list">${events.length ? events.slice(0, 40).map((event) => `
          <article class="so-event ${esc(event.kind)}">
            ...
            <div><strong>${esc(event.name)}</strong><small>${esc(event.systemName)} • [${event.x}, ${event.y}, ${event.z}]${event.status ? ` • ${esc(event.status)}` : ""}</small><small>${esc(countdownLabel(event.endsAt))}${event.weakness ? ` • Weakness: ${esc(cleanLabel(event.weakness))}` : ""}</small></div>
            <button data-action="copy-coordinate" data-coordinate="${event.x}, ${event.y}, ${event.z}" title="Copy coordinates">Copy</button>
          </article>`).join("") : `<div class="so-empty">No active official events are cached. Connect or refresh the official API in Sync.</div>`}</div>
      </section>
      <section><h3>Perfect-node finder</h3>
        ...
        <p class="so-note">${state.officialApi.jumps ? `Journal: ${compactNumber(state.officialApi.jumps)} jumps across ${compactNumber(state.officialApi.journalSystems)} coordinates. ` : ""}Routes use a nearest-next heuristic on your current Z layer. Node data comes only from systems observed by players using the companion.</p>
        <div class="so-route-list">${route.length ? route.map((system, index) => `
          <article class="so-route-row"><b>${index + 1}</b><div><strong>${esc(system.name)}</strong><span>[${system.x}, ${system.y}, ${system.z}]${system.distance !== null ? ` • ${system.distance.toFixed(1)} units from prior stop` : ""}</span></div><button data-action="copy-coordinate" data-coordinate="${system.x}, ${system.y}, ${system.z}">Copy</button></article>`).join("") : `<div class="so-empty">No matching perfect systems are available on this map layer.</div>`}</div>
      </section>
      <div class="so-list">
        ${rows.length ? rows.slice(0, 150).map((system) => `
          <article class="so-row">
            <div class="so-row-main"><strong>${esc(system.name)}</strong><span>[${system.x}, ${system.y}, ${system.z}]</span></div>
  ...
  const renderRoster = () => {
    ...
    return `
      ...
      ${stations.length ? `<section><h3>Squadron stations</h3><div class="so-list">${stations.map((station) => `<article class="so-row"><div class="so-row-main"><strong>${esc(station.name)}</strong><span>[${station.x}, ${station.y}, ${station.z}]</span></div><div class="so-inline-actions"><button data-action="copy-coordinate" data-coordinate="${station.x}, ${station.y}, ${station.z}">Copy coordinates</button></div></article>`).join("")}</div></section>` : ""}
      ${Object.keys(bonuses).length || buildings.length ? `<section><h3>Squadron capability</h3>${Object.keys(bonuses).length ? `<div class="so-grid">${renderPairs(bonuses)}</div>` : ""}${buildings.length ? `<div class="so-chips muted">${buildings.slice(0, 30).map((building) => `<span>${esc(building.name || building.building || "Building")}${building.level ? ` • L${esc(building.level)}` : ""}</span>`).join("")}</div>` : ""}</section>` : ""}
  ...
  const xpTrendForKey = (key, activity, windowName) => {
    ...
    if (!baseline?.activities?.[activity] || !sharedLatest?.activities?.[activity]) return local;
      ...
      <section class="so-xp-leaderboard-card">
        ...
        <div class="so-xp-leaderboard-list">
          ${board.ranked.length ? board.ranked.map((row) => {
            const confidence = confidenceForTrend(row.trend, XP_MIN_RATE_WINDOW_MS);
            return `
        ...
        <div class="so-xp-leaderboard-foot">${board.ranked.length} ranked • ${board.calibrating.filter((row) => !row.trend.withheld).length} calibrating • ${board.calibrating.filter((row) => row.trend.withheld).length} withheld for outliers • minimum 10 minutes</div>
      </section>`;
  };
  ...
  const renderXpRankings = () => {
    ...
    return `
      ...
      <div class="so-xp-leaderboards">
        <p class="so-note">Rankings include you, squad members, and favorited profiles. The selected window applies to XP/hour and RSS/hour; RSS rates use cumulative profile totals only.</p>
      ...
      <p class="so-note">XP rates need two comparable snapshots spanning 10 minutes. RSS rates require profile snapshots spanning at least 60 minutes; the game leaderboard is retained only for official rank display. Reopen profiles normally to refresh their totals. Ties share the same place.</p>`;
  };
  ...
  const combatStat = (profile, stat) => {
    ...
    const match = Object.entries(values).find(([key, value]) => key.toLowerCase().endsWith(`_${stat}`) && finite(value) !== null);
    return match ? finite(match[1]) : 0;
  };

  const simulatedBuild = (profile, mods) => Object.fromEntries(["power", "precision", "evasion", "hull"].map((stat) => {
    const base = combatStat(profile, stat);
    return [stat, base * (1 + (finite(mods?.[stat]) || 0) / 100)];
  }));
  ...
  const pvpEstimate = (leftProfile, rightProfile) => {
    ...
    const score = (attacker, defender) => {
      const hitChance = Math.max(0.1, Math.min(0.95, attacker.precision / Math.max(1, attacker.precision + defender.evasion)));
      return Math.max(1, attacker.power) * hitChance * Math.max(1, attacker.hull);
    };
    const leftScore = score(left, right);
    const rightScore = score(right, left);
    return { left, right, chance: leftScore / Math.max(1, leftScore + rightScore) * 100 };
  };

  const renderBuildLab = (keys) => {
    const buildProfile = (key) => state.profiles[key] || state.buildLab.imports[key];
    const usable = [...new Set([...keys.filter((key) => state.profiles[key] && !state.profiles[key].placeholder), ...Object.keys(state.buildLab.imports)])];
    ...
    const modifiers = (side, values) => `<div class="so-build-mods">${["power", "precision", "evasion", "hull"].map((stat) => `<label>${cleanLabel(stat)} %<input type="number" step="1" min="-90" max="500" data-action="build-mod" data-side="${side}" data-stat="${stat}" value="${esc(values[stat] || 0)}"></label>`).join("")}</div>`;
    return `<section><h3>Build lab & PvP sandbox</h3>
      <p class="so-note">Compare observed profiles, then test hypothetical percentage bonuses. This is a transparent relative estimate—not a promise of an outcome and not a game action.</p>
      ...
      <div class="so-versus"><div><strong>${esc(leftProfile.username)}</strong><span>${result.chance.toFixed(1)}% model share</span></div><b>VS</b><div><strong>${esc(rightProfile.username)}</strong><span>${(100 - result.chance).toFixed(1)}% model share</span></div></div>
      ...
      <div class="so-grid">${["power", "precision", "evasion", "hull"].map((stat) => `<div class="so-pair"><span>${cleanLabel(stat)}</span><strong>${compactNumber(result.left[stat])} / ${compactNumber(result.right[stat])}</strong></div>`).join("")}</div>
  ...
  const ensureUi = () => {
    ...
    root.addEventListener("click", (event) => {
      ...
      if (action === "toggle") {
        ...
        const profile = state.profiles[state.buildLab.left] || state.buildLab.imports[state.buildLab.left];
        if (profile) {
          const payload = { v: 1, player: profile.username, stats: Object.fromEntries(["power", "precision", "evasion", "hull"].map((stat) => [stat, combatStat(profile, stat)])), gear: (profile.gear || []).map((item) => ({ slot: item.slot, name: item.name, catalysts: item.catalysts })), mods: state.buildLab.leftMods };
        ...
        try {
          ...
          state.buildLab.imports[key] = { username: String(payload.player).slice(0, 64), stats: Object.fromEntries(["power", "precision", "evasion", "hull"].map((stat) => [stat, finite(payload.stats[stat]) || 0])), gear: Array.isArray(payload.gear) ? payload.gear.slice(0, 12) : [], placeholder: false, source: "imported build code", capturedAt: Date.now() };
    ...
    root.addEventListener("change", (event) => {
      if (event.target.matches('[data-action="select-profile"]')) {
        ...
        if (["power", "precision", "evasion", "hull"].includes(stat)) state.buildLab[side][stat] = Math.min(500, Math.max(-90, finite(event.target.value) || 0));
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
  ...
  const renderPanel = () => {
    ...
    const signature = JSON.stringify({
      ...
      galaxyTools: [state.nodeFinder, state.waypoints, state.officialApi.dungeons, state.officialApi.soloDungeons, state.officialApi.activeRunes],
  ...
  const renderMapMarkers = () => {
    ...
    for (const event of allGalaxyEvents()) {
      ...
      markers.push({
        ...
        label: event.kind === "rune" ? "R" : event.kind === "solo" ? "S" : "D",
