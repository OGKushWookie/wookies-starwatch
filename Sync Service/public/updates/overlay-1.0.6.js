(() => {
  ...
  const levelEtaLabel = (milliseconds) => {
    ...
    return days >= 100 ? "99d+" : remainingHours ? `${days}d ${remainingHours}h` : `${days}d`;
  };

  const renderHeaderLevelEtas = () => {
    const cards = [...document.querySelectorAll("#q-app .q-header .mainCardsClass > .q-item")];
    if (!cards.length) return;
    const profile = state.profiles[state.selfProfileKey];
    const currentActivities = xpActivitiesFromLevels(profile?.levels || {});
    const history = state.xpHistory[state.selfProfileKey] || [];

    for (const activity of ["battling", "gathering"]) {
      const card = cards.find((candidate) => String(candidate.textContent || "").toLowerCase().includes(activity));
      if (!card) continue;
      const xpRow = [...card.querySelectorAll(".flex.row.justify-between.items-center")]
        .find((candidate) => /\bXP\s*:/i.test(candidate.textContent || ""));
      if (!xpRow) continue;

      let eta = xpRow.querySelector(".so-intel-header-eta");
      if (!eta) {
        eta = document.createElement("span");
        eta.className = "so-intel-header-eta";
        xpRow.appendChild(eta);
      }

      const current = currentActivities[activity];
      const trend = xpTrend(history, activity, "24h");
      const remaining = current && current[2] > 0 ? Math.max(0, current[2] - current[1]) : null;
      const ready = remaining !== null && trend.rate > 0;
      eta.textContent = ready ? `ETA: ${levelEtaLabel((remaining / trend.rate) * 3600000)}` : "ETA: calibrating";
      eta.title = ready
        ? `Level-up estimate from ${xpRateNumber(trend.rate)} observed XP/hour over the latest 24-hour window`
        : "Needs at least 10 minutes of comparable XP observations";
    }
  };
  ...
  const renderSync = () => {
    ...
    return `
      <div class="so-sync">
        ...
        <p class="so-note">Favorites, systems seen, and 100% node systems follow the Steam-linked account. Public profile, XP, and RSS observations are pooled across participating users only after they enable shared history, and only when the game has already loaded that information normally.</p>
  ...
  const renderAbout = () => `
    <div class="so-about">
      ...
      <p>XP rates are wall-clock estimates from repeated snapshots. No profile or map location is polled automatically. Optional shared sync talks only to the separately configured companion service, never to the game server.</p>
      <h3>Map legend</h3>
  ...
  const ensureUi = () => {
    ...
    style.textContent = `
      @media(min-width:1101px){
        ...
        html.${DRAWER_LAYOUT_CLASS}.${DRAWER_OPEN_CLASS} #q-app .q-header>.q-toolbar{min-height:64px!important;align-items:center!important}
        html.${DRAWER_LAYOUT_CLASS}.${DRAWER_OPEN_CLASS} #q-app .q-header>.q-toolbar>.q-list.gt-sm{display:grid!important;grid-template-columns:repeat(2,34px);grid-template-rows:repeat(2,32px);flex:0 0 68px!important;width:68px!important;max-width:68px!important;align-self:stretch;align-content:center;justify-content:center;overflow:visible}
        html.${DRAWER_LAYOUT_CLASS}.${DRAWER_OPEN_CLASS} #q-app .q-header>.q-toolbar>.q-list.gt-sm>.q-item{width:34px!important;min-width:34px!important;min-height:32px!important;padding-left:4px!important;padding-right:4px!important}
      ...
      #q-app .so-intel-header-eta{flex:0 0 auto;margin-left:8px;color:#e1b94f;font-size:11px;font-weight:500;white-space:nowrap;text-transform:none}
  ...
  window.__stellarOdysseyIntelOverlay = {
    ...
    destroy() {
      ...
      document.getElementById(ENGINE_TOAST_ID)?.remove();
      document.getElementById(ENGINE_FLASH_ID)?.remove();
      document.querySelectorAll("#q-app .so-intel-header-eta").forEach((element) => element.remove());
