(() => {
  ...
  const bootstrapSyncOutbox = () => {
    ...
    Object.keys(state.seen).forEach(queueSyncSystem);
  ...
  if (state.sync.enabled && !state.sync.outboxXp.length && !state.sync.outboxResources.length && !Object.keys(state.sync.outboxProfiles).length && !state.sync.outboxSystems.length) bootstrapSyncOutbox();
