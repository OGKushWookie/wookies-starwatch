(() => {
  ...
  const resourceTrend = (history, windowName) => {
    ...
    return {
      ...
      rate: gained >= 0 && elapsed >= RESOURCE_MIN_RATE_WINDOW_MS ? gained / (elapsed / 3600000) : null,
      sampleCount: samples.length,
    };
  };
  ...
  const renderResourceLeaderboard = (keys) => {
    ...
    return `
      <section class="so-xp-leaderboard-card so-resource-leaderboard-card">
        ...
        <div class="so-xp-leaderboard-foot">${ranked.length} ranked • ${calibrating.length} calibrating • profile totals only • minimum 60 minutes observed</div>
      </section>`;
  };
