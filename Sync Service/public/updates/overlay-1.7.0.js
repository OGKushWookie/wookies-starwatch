(() => {
  ...
  const routeGameContext = () => {
    ...
    return {
      ...
      totalFuel: finite(explore?.total_fuel),
  ...
  const plannedRoute = () => {
    ...
    if (origin.x === null || origin.y === null || !destinations.length) return { context, origin, destinations, order: [], legs: [], distance: 0, fuel: 0, portalCredits: 0, arrivalWait: context.engineWait, readyWait: context.engineWait };
    ...
    return {
      ...
      order,
      legs,
      distance: segments.reduce((sum, segment) => sum + (segment.type === "travel" ? segment.distance : 0), 0),
      fuel: segments.reduce((sum, segment) => sum + (segment.type === "travel" ? segment.fuel : 0), 0),
  ...
  const routeLegDescription = (leg) => leg.segments.map((segment) => segment.type === "teleport"
    ? `Teleport ${segment.from.name} → ${segment.to.name} (${compactNumber(segment.credits)} credits)`
    : `Fly ${segment.distance.toFixed(1)} ly${segment.to?.name ? ` to ${segment.to.name}` : ""}`
  ).join(" • ");
  ...
  const renderNodes = () => {
    ...
    const fuelShortfall = plan.context.currentFuel === null ? null : Math.max(0, plan.fuel - plan.context.currentFuel);
      ...
      <section><div class="so-section-heading"><h3>Shortest route planner</h3>${state.routePlanner.destinations.length ? `<button data-action="clear-route">Clear route</button>` : ""}</div>
        ...
        ${plan.order.length ? `
          <div class="so-route-summary">
            ...
            <div><span>Fuel required</span><strong>${plan.fuel.toFixed(1)}</strong><small>${plan.context.currentFuel === null ? "live fuel unavailable" : `${plan.context.currentFuel.toFixed(1)} currently`}</small></div>
          ...
          ${fuelShortfall > 0 ? `<div class="so-route-warning">Route needs ${fuelShortfall.toFixed(1)} more fuel than is currently loaded. Refueling actions are not estimated or automated.</div>` : ""}
          ...
          <div class="so-route-list">${plan.legs.map((leg, index) => `
            <article class="so-route-row"><b>${index + 1}</b><div><strong>${esc(leg.destination.name)}</strong><span>[${leg.destination.x}, ${leg.destination.y}, ${leg.destination.z}] • ${esc(routeLegDescription(leg))}</span></div><button data-action="remove-route-stop" data-route-key="${esc(leg.destination.key)}">Remove</button></article>
          `).join("")}</div>
          <p class="so-route-footnote">Fuel uses the game’s distance formula and your live capped ${plan.context.fuelEfficiency.toFixed(1)}% fuel-efficiency bonus. Time uses the live ${plan.context.cooldownReduction.toFixed(1)}% cooldown reduction and current engine timer. Portal costs use the departure station’s live portal level.</p>
  ...
  const ensureUi = () => {
    ...
    root.addEventListener("click", (event) => {
      ...
      if (action === "toggle") {
        ...
        state.routePlanner = { destinations: [], usePortals: true };
        state.operations = {};
