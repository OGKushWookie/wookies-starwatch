(() => {
  ...
  const publicAtlas = { supported: false, systems: [], totalSystems: 0, fetchedAt: 0, checkedAt: 0, refreshing: false, stale: false, hasSnapshot: false, message: "Checking the public index…" };
  const ROOT_ID = "so-intel-overlay";
  const MAP_LAYER_ID = "so-intel-map-layer";
  const MINI_MAP_ROUTE_CLASS = "so-intel-minimap-route";
  ...
  const ROUTE_MAX_DESTINATIONS = 8;
  const GALAXY_MAX_COORDINATE = 7000;
  const XP_WINDOWS = { "6h": 6 * 60 * 60 * 1000, "24h": 24 * 60 * 60 * 1000, "7d": 7 * 24 * 60 * 60 * 1000, all: Infinity };
  ...
  const HISTORY_SYNC_VERSION = 3;
  const RESOURCE_HISTORY_VERSION = 2;
  const DEFAULT_SYNC_ENDPOINT = "https://stellar-odyssey-intel-sync.sthess28.workers.dev";
  const TAB_META = {
    nodes: ["Galaxy Intel", "Events, perfect resources and portal-aware routes"],
    dust: ["Dust Atlas", "Base discovery value • nearest-starter heatmap"],
  ...
  const savedRouteDestinations = (Array.isArray(saved.routePlanner?.destinations) ? saved.routePlanner.destinations : [])
    .map((row) => ({
      key: String(row?.key || `${Number(row?.z ?? 1)}:${Number(row?.x)},${Number(row?.y)}`),
      name: String(row?.name || "Destination").slice(0, 100),
      kind: String(row?.kind || "saved").slice(0, 30),
      x: Number(row?.x),
      y: Number(row?.y),
      z: Number(row?.z ?? 1),
    ...
    .slice(-ROUTE_MAX_DESTINATIONS);
  const migrateResourceHistory = Number(saved.sync?.resourceHistoryVersion || 0) < RESOURCE_HISTORY_VERSION;
  const migrateHistorySync = Number(saved.sync?.historySyncVersion || 0) < HISTORY_SYNC_VERSION;
  ...
  const state = {
    ...
    nodeFinder: {
      ...
      resource: ["rocky", "icy", "gas", "crystal"].includes(saved.nodeFinder?.resource) ? saved.nodeFinder.resource : "all",
    },
    routePlanner: {
      destinations: savedRouteDestinations,
      usePortals: saved.routePlanner?.usePortals !== false,
      showOnMaps: saved.routePlanner?.showOnMaps !== false,
      expandedLeg: String(saved.routePlanner?.expandedLeg || ""),
    },
    ...
    pvmLab: {
      ...
      engineStatus: "idle",
      engineSource: String(saved.pvmLab?.engineSource || ""),
      job: "idle",
      progress: 0,
    ...
    sync: {
      ...
      summaryPulledAt: migrateResourceHistory || migrateHistorySync ? 0 : Number(saved.sync?.summaryPulledAt || 0),
      summaryPlayerSet: String(saved.sync?.summaryPlayerSet || ""),
      historyCursors: !migrateResourceHistory && !migrateHistorySync && saved.sync?.historyCursors && typeof saved.sync.historyCursors === "object" ? saved.sync.historyCursors : {},
      ...
      outboxSystems: Array.isArray(saved.sync?.outboxSystems)
        ...
        : [],
      // Completed-battle contribution is a separate privacy choice. New
      // installations must opt in; an existing saved choice is preserved.
      battleSharing: saved.sync?.battleSharing === true,
      privacyAcceptedAt: Number(saved.sync?.privacyAcceptedAt || ((saved.sync?.enabled === true && String(saved.sync?.token || "").length >= 32) ? Date.now() : 0)),
  ...
  const persist = () => {
    ...
    try {
      localStorage.setItem(STORE_KEY, JSON.stringify({
        ...
        dustCatalog: [...dustCatalog.entries()],
        dustView: dustView.initialized ? { x: dustView.x, y: dustView.y, span: dustView.span } : null,
        ...
        pvmLab: {
          ...
          engineSource: state.pvmLab.engineSource,
        },
        officialApi: {
        ...
        sync: {
          ...
          privacyAcceptedAt: state.sync.privacyAcceptedAt,
      ...
      console.warn("[Starwatch] Could not persist local cache", error);
    }
  };
  ...
  const pvmCurrentBuild = () => {
    ...
    try {
      ...
      console.warn("[Starwatch] Could not read the current catalyst bonus summary", error);
    }
  ...
  const pvmIsEngine = (library) => Boolean(library
    && ["simulateBattle", "getWinrate", "findMaxLevelAtThreshold", "findOptimalBuild"].every((name) => typeof library[name] === "function")
    && Array.isArray(library.battlingNPCs) && library.battlingNPCs.length > 0
    && library.battlingNPCs.every((npc) => typeof npc?.name === "string" && Array.isArray(npc.weakness)));

  const pvmLibraryExport = (source) => {
    // Recognize the game's CommonJS stellarlib entry wrapper and trace its ESM
    // export. Minified names change each build. Do not call arbitrary exports,
    // evaluate source text, or assume the old alias still identifies the library.
    const id = "[A-Za-z_$][\\w$]*";
    const wrapper = new RegExp([
      `(?<factory>${id})\\s*=\\s*${id}\\(\\(\\(\\s*${id}\\s*,\\s*(?<module>${id})\\s*\\)\\s*=>\\s*\\{\\s*`,
      `(?:var|let|const)\\s+(?<library>${id})\\s*=\\s*${id}\\(\\s*\\)\\s*;\\s*`,
      "\\k<module>\\.exports\\s*=\\s*\\k<library>\\s*,\\s*",
      "\\k<module>\\.exports\\.default\\s*=\\s*\\k<library>\\s*,\\s*",
      "\\k<module>\\.exports\\.stellarlib\\s*=\\s*\\k<library>\\s*;?\\s*\\}\\s*\\)\\s*\\)",
    ].join(""), "g");
    const factories = new Set([...source.matchAll(wrapper)].map((match) => match.groups.factory));
    const aliases = new Set();
    for (const block of source.matchAll(/\bexport\s*\{([^}]+)\}\s*;?/g)) {
      for (const entry of block[1].split(",")) {
        const match = entry.trim().match(new RegExp(`^(${id})(?:\\s+as\\s+(${id}))?$`));
        if (match && factories.has(match[1])) aliases.add(match[2] || match[1]);
      }
    }
    if (aliases.size !== 1) throw new Error("The game updated its local combat-library interface; a compatibility update is needed");
    return [...aliases][0];
  };

  const pvmReadLocalBundle = async (mainUrl) => {
    const url = new URL(mainUrl);
    // Steam's installed assets only. Never fetch game-server code, API data,
    // routes, or lazy game pages to discover the simulator.
    if (url.protocol !== "file:" || url.host || !/\/assets\/index-[^/]+\.js$/.test(url.pathname)) {
      throw new Error("The NPC simulator requires the installed Steam client's local game files");
    }
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 5000);
    try {
      const response = await fetch(url.href, { signal: controller.signal, cache: "force-cache" });
      if (!response.ok) throw new Error("The installed NPC simulator bundle could not be read");
      const source = await response.text();
      if (source.length > 8 * 1024 * 1024) throw new Error("The game client bundle format changed; a compatibility update is needed");
      return source;
    } finally {
  ...
  const loadPvmEngine = async () => {
    ...
    pvmEnginePromise = (async () => {
      ...
      const engineSource = decodeURIComponent(mainUrl.split("/").pop()?.split("?")[0] || "game client");
      if (state.pvmLab.engineSource !== engineSource) {
        // Predictions from another game version must not appear current, even
        // if this version cannot load the new engine yet. Keep editable builds.
        state.pvmLab.result = null;
        state.pvmLab.maxResult = null;
        state.pvmLab.optimizer = null;
        markDirty();
      }
      const source = await pvmReadLocalBundle(mainUrl);
      const exportName = pvmLibraryExport(source);
      const gameModule = await import(mainUrl);
      const library = typeof gameModule[exportName] === "function" ? gameModule[exportName]() : null;
      if (!pvmIsEngine(library)) {
        throw new Error("The game updated its local combat-library interface");
      }
      state.pvmLab.engineStatus = "ready";
      state.pvmLab.engineSource = engineSource;
  ...
  const xpConfigurationContext = (playerKey) => {
    const context = { v: 1 };
    try {
      const boostStore = getPinia()?._s?.get("GlobalBoostsStore");
      if (boostStore) {
        const active = Array.from(boostStore.currentBoosts || []).find((entry) => entry?.boost?.name === "XP");
        const tier = Array.from(boostStore.GlobalBoostTier || []).find((entry) => entry?.boost === "XP");
        const enabled = Number(active?.timer || 0) > Math.floor(Date.now() / 1000);
        context.b = enabled ? Math.max(0, finite(tier?.bonus) || 0) : 0;
        context.t = enabled ? Math.max(0, finite(tier?.tier) || 0) : 0;
      }
    } catch {
      // Configuration metadata is optional and must never block an XP observation.
    }
    const profile = state.profiles[String(playerKey || "").toLowerCase()];
    const gear = (Array.isArray(profile?.gear) ? profile.gear : []).map((item) => ({
      s: String(item?.slot || ""),
      n: String(item?.name || ""),
      l: finite(item?.level),
      c: (Array.isArray(item?.catalysts) ? item.catalysts : []).map((catalyst) => [
        String(catalyst?.stat || ""),
        String(catalyst?.bonus || ""),
        finite(catalyst?.range),
      ]).sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b))),
    })).sort((a, b) => a.s.localeCompare(b.s));
    const pets = (Array.isArray(profile?.pets) ? profile.pets : [])
      .filter((pet) => pet?.active)
      .map((pet) => [String(pet?.name || ""), finite(pet?.level), String(pet?.xpboost ?? "")])
      .sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b)));
    if (gear.length || pets.length) context.g = shortFingerprint(JSON.stringify({ gear, pets }));
    return Object.keys(context).length > 1 ? context : null;
  };

  let xpRequirementCache = null;
  const invalidateXpRequirementCache = () => { xpRequirementCache = null; };
  const xpRequirementTable = (activity) => {
    if (!xpRequirementCache) {
      xpRequirementCache = Object.fromEntries(XP_ACTIVITIES.map((name) => [name, new Map()]));
      const addActivities = (activities) => {
        for (const name of XP_ACTIVITIES) {
          const row = activities?.[name];
          const level = finite(row?.[0]);
          const target = finite(row?.[2]);
          if (level !== null && target !== null && target > 0) xpRequirementCache[name].set(level, target);
        }
      };
      for (const history of Object.values(state.xpHistory || {})) for (const sample of Array.isArray(history) ? history : []) addActivities(sample?.activities);
      for (const summary of Object.values(state.sharedSummaries?.xp || {})) {
        addActivities(summary?.latest?.activities);
        for (const baseline of Object.values(summary?.baselines || {})) addActivities(baseline?.activities);
      }
      for (const profile of Object.values(state.profiles || {})) addActivities(xpActivitiesFromLevels(profile?.levels || {}));
    }
    return xpRequirementCache[activity] || new Map();
  };

  const xpContextKey = (sample) => {
    const context = sample?.activities?._context;
    if (!context || typeof context !== "object") return "";
    const parts = [];
    if (Number.isFinite(context.b)) parts.push(`b:${context.b}`);
    if (Number.isFinite(context.t)) parts.push(`t:${context.t}`);
    if (typeof context.g === "string" && context.g) parts.push(`g:${context.g}`);
    return parts.join("|");
  };

  const latestCompatibleXpSegment = (samples) => {
    if (samples.length < 2) return { samples, configurationChanged: false };
    const runs = [];
    for (let index = 0; index < samples.length; index += 1) {
      const key = xpContextKey(samples[index]);
      if (!key) continue;
      const current = runs.at(-1);
      if (current?.key === key) current.count += 1;
      else runs.push({ key, start: index, count: 1 });
    }
    const latest = runs.at(-1);
    const previous = runs.at(-2);
    // The client can briefly expose a partial loadout while its stores hydrate. Require
    // both configurations to repeat before treating the difference as a real change.
    const confirmed = Boolean(latest?.count >= 2 && previous?.count >= 2);
    return { samples: confirmed ? samples.slice(latest.start) : samples, configurationChanged: confirmed };
  };
  ...
  const beginSteamLink = async ({ requestId = "", authorizeUrl = "", opened = false } = {}) => {
    ...
    if (!state.sync.privacyAcceptedAt) {
      state.sync.status = "setup";
      state.sync.message = "Review and accept the shared-data notice before connecting Steam";
      state.lastPanelSignature = "";
      renderPanel();
      return;
    }
  ...
  const deleteCloudAccount = async () => {
    ...
    state.sync.authBusy = true;
    state.sync.status = "linking";
    state.sync.message = "Deleting this linked account from the companion service";
    state.lastPanelSignature = "";
    renderPanel();
    try {
      const result = await companionJson("/v1/account/delete", {
        owner: syncOwner(),
        token: state.sync.token,
        confirmation: "DELETE",
      }, 30000);
      ...
      state.sync.linkRequest = null;
      state.sync.enabled = false;
      state.sync.battleSharing = false;
      state.sync.privacyAcceptedAt = 0;
      state.sync.outboxXp = [];
      state.sync.outboxResources = [];
      state.sync.outboxProfiles = {};
      state.sync.outboxSystems = [];
      state.sync.outboxBattles = [];
      state.sync.summaryPulledAt = 0;
      state.sync.summaryPlayerSet = "";
      state.sync.historyCursors = {};
      state.sync.status = "idle";
      state.sync.message = result.aggregateRetention
        ? "Cloud account deleted • anonymous pooled statistics may remain"
        : "Cloud account deleted";
      markDirty();
      persist();
      showEngineToast("Linked cloud account deleted; local overlay data was kept", true);
    } catch (error) {
      state.sync.status = "error";
      state.sync.message = `Cloud deletion failed: ${String(error?.message || error).slice(0, 120)}`;
    } finally {
      state.sync.authBusy = false;
      state.lastPanelSignature = "";
      renderPanel();
  ...
  const diagnosticReport = () => ({
    report: "Wookie's Starwatch diagnostics",
    generatedAt: new Date().toISOString(),
    overlayVersion: VERSION,
    launcherVersion: state.officialApi.launcherVersion || "unavailable",
    privateBeta: true,
    view: {
      tab: state.tab,
    ...
    officialApi: {
      bridgeAvailable: state.officialApi.available,
      configured: state.officialApi.configured,
      status: state.officialApi.status,
      lastRefreshAt: state.officialApi.lastRefreshAt || 0,
      cachedRoutes: Object.fromEntries(Object.entries(state.officialApi.caches || {}).map(([key, row]) => [key, {
    ...
    sharedSync: {
      consentAccepted: Boolean(state.sync.privacyAcceptedAt),
      enabled: state.sync.enabled,
      authentication: state.sync.authMode,
      deviceConnected: state.sync.authMode === "steam" && state.sync.token.length >= 32,
      status: state.sync.status,
      failureCount: state.sync.failureCount,
      lastSuccessfulPullAt: state.sync.lastPullAt || 0,
      nextAttemptAt: state.sync.nextAttemptAt || 0,
      queued: {
        xp: state.sync.outboxXp.length,
        resources: state.sync.outboxResources.length,
        profiles: Object.keys(state.sync.outboxProfiles).length,
        systems: state.sync.outboxSystems.length,
        battles: state.sync.outboxBattles.length,
      },
      battleContributionEnabled: state.sync.battleSharing,
      battlesCapturedOnDevice: state.sync.battleCaptured,
      learnedModelCounts: Object.fromEntries(Object.entries(state.sync.battleModel || {}).map(([key, row]) => [key, Number(row?.battles || 0)])),
    },
    localIndexCounts: {
      systems: Object.keys(state.systems).length,
      verifiedPerfectSystems: verifiedPerfectCount(),
      profiles: Object.keys(state.profiles).length,
      favorites: state.favorites.size,
      xpPlayers: Object.keys(state.xpHistory).length,
      resourcePlayers: Object.keys(state.resourceHistory).length,
    },
    pvm: {
      engineStatus: state.pvmLab.engineStatus,
      buildLoaded: Boolean(state.pvmLab.loadedAt),
      lastJobStatus: state.pvmLab.job,
    },
    privacy: "No player names, coordinates, API keys, device credentials, recovery codes, profile contents, or raw observations are included.",
  });
  ...
  const copyDiagnosticReport = async () => {
    ...
    try {
      ...
      showEngineToast("Privacy-safe diagnostic report copied", true);
    } catch {
      ...
      field.remove();
      showEngineToast(copied ? "Privacy-safe diagnostic report copied" : "Diagnostic copy was blocked", true);
    }
  };
  ...
  const captureBattleObservations = async () => {
    ...
    try {
      ...
      console.warn("[Starwatch] Could not capture completed battle", error);
    } finally {
  ...
  const recordXpSnapshot = (username, levels, source, capturedAt = Date.now()) => {
    ...
    const context = xpConfigurationContext(key);
    if (context) activities._context = context;
  ...
  const xpDelta = (first, last, activity) => {
    ...
    const requirements = xpRequirementTable(activity);
    const firstRequirement = startTarget > 0 ? startTarget : requirements.get(startLevel);
    if (!(firstRequirement > 0)) return null;
    let gained = Math.max(0, firstRequirement - startCurrent) + Math.max(0, endCurrent);
    let estimated = false;
    let missingLevels = 0;
    for (let level = startLevel + 1; level < endLevel; level += 1) {
      const exact = requirements.get(level);
      if (exact > 0) {
        gained += exact;
      } else {
        missingLevels += 1;
        estimated = true;
        const fallbackTarget = endTarget > 0 ? (firstRequirement + endTarget) / 2 : firstRequirement;
        gained += fallbackTarget;
      }
    }
    return {
      gained,
      confidence: estimated ? "low" : "medium",
      estimated,
      missingLevels,
    };
  };
  ...
  const captureResponse = (url, payload) => {
    ...
    try {
      ...
      console.debug("[Starwatch] Ignored an unrecognized response shape", error);
    }
  };
  ...
  const ingestLiveClientState = () => {
    ...
    captureDustSystem(pinia._s.get("ExploreStore")?.currentSystem);
  ...
  const applyOfficialJournal = (result) => {
    ...
    for (const entry of rows) {
      ...
      const key = `${z}:${x},${y}`;
      const date = finite(entry?.date);
      const visitedAt = date !== null && date > 0
        ? Math.min(date < 1e11 ? date * 1000 : date, Number(result?.fetchedAt || Date.now()))
        : Number(result?.fetchedAt || Date.now());
    ...
    // The journal is personal history. Its bodies can be ingested as observations;
    // the global public atlas deliberately never passes through ingestSystems.
    ingestSystems(rows.filter((entry) => Array.isArray(entry?.bodies)).map((entry) => ({ ...entry, name: entry.systemName || entry.name })));
  ...
  let atlasPollTimer = 0;
  let atlasRequestPending = false;
  let atlasDisposed = false;
  let atlasGeneration = 0;
  const resetPerfectIndex = () => {
    atlasGeneration++;
    publicAtlas.systems = [];
    publicAtlas.fetchedAt = publicAtlas.checkedAt = publicAtlas.totalSystems = 0;
    publicAtlas.hasSnapshot = publicAtlas.refreshing = publicAtlas.stale = false;
    publicAtlas.message = "Add your official API key in Sync to load the public index.";
    nodeDataRevision++;
  };
  const refreshPerfectIndex = async () => {
    if (atlasDisposed || atlasRequestPending || !publicAtlas.supported || !state.officialApi.configured) return;
    atlasRequestPending = true;
    const generation = atlasGeneration;
    clearTimeout(atlasPollTimer);
    try {
      // This is a small local snapshot; the helper schedules any large API work independently.
      const result = await nativeBridgeJson("/official/systems", { timeoutMs: 5000 });
      if (atlasDisposed || generation !== atlasGeneration) return;
      if (result.hasSnapshot && (!publicAtlas.hasSnapshot || Number(result.fetchedAt) !== publicAtlas.fetchedAt)) {
        const rows = result.data?.systems;
        if (!Array.isArray(rows) || rows.length > 25000) throw new Error("The public index response is invalid.");
        publicAtlas.systems = rows.map((row) => ({
          name: String(row?.name || "Unknown system").slice(0, 100),
          x: finite(row?.x), y: finite(row?.y), z: finite(row?.z ?? 1),
          nodes: (Array.isArray(row?.nodes) ? row.nodes : []).filter((node) => typeof node?.quality === "number" && node.quality === 100).map((node) => ({
            id: String(node.id || "").slice(0, 100), type: String(node.type || "resource").slice(0, 40), body: String(node.body || "Body").slice(0, 80), quality: 100,
          })),
          source: "Public API", seenAt: Number(result.fetchedAt), verifiedAt: Number(result.fetchedAt),
        })).filter((row) => row.x !== null && row.y !== null && row.z === 1 && row.nodes.length);
        publicAtlas.totalSystems = Math.max(0, Number(result.data?.totalSystems || 0));
        publicAtlas.fetchedAt = Number(result.fetchedAt || 0);
        publicAtlas.hasSnapshot = true;
        nodeDataRevision++;
      }
      publicAtlas.checkedAt = Number(result.checkedAt || 0);
      publicAtlas.refreshing = result.refreshing === true;
      publicAtlas.stale = result.stale === true;
      publicAtlas.message = String(result.message || "").slice(0, 200);
    } catch (error) {
      if (atlasDisposed || generation !== atlasGeneration) return;
      publicAtlas.stale = publicAtlas.hasSnapshot;
      publicAtlas.refreshing = false;
      publicAtlas.message = String(error?.message || "Public index unavailable").slice(0, 200);
    } finally {
      atlasRequestPending = false;
      if (!atlasDisposed) {
        state.lastPanelSignature = "";
        if (state.open && state.tab === "nodes") renderPanel();
        atlasPollTimer = setTimeout(refreshPerfectIndex, publicAtlas.refreshing ? 3000 : 60000);
      }
    }
  };
  ...
  const refreshOfficialStatus = async () => {
    ...
    publicAtlas.supported = result.perfectIndex === true;
    if (!publicAtlas.supported) publicAtlas.message = "Install launcher 2.2.0 to search the public galaxy. Personal observations are still available.";
  ...
  const mergeSharedSystems = (rows) => {
    ...
    if (changed) { nodeDataRevision++; markDirty(); }
  };
  ...
  const runSharedSync = async (force = false) => {
    ...
    if (!state.sync.privacyAcceptedAt) {
      state.sync.status = "setup";
      state.sync.message = "Shared sync is paused until the data notice is accepted";
      if (force) {
        state.lastPanelSignature = "";
        renderPanel();
      }
  ...
  const confidenceForTrend = (trend, minimumMs) => {
    ...
    const method = String(trend.method || "observed trend");
    const exactLabel = trend.delta
      ? trend.delta.estimated
        ? ` • ${trend.delta.missingLevels || 1} level requirement${trend.delta.missingLevels === 1 ? "" : "s"} estimated`
        : " • exact observed level requirements"
      : "";
    ...
    if (trend.staleBeyondWindow) return { level: "pending", label: "Outside window", detail: `Last observation was ${ageLabel(trend.latest.at)} ago` };
    if (trend.rate === null || span < minimumMs) return { level: "pending", label: trend.configurationChanged ? "New configuration" : "Calibrating", detail: `${samples} samples • ${durationLabel(span)} compatible span` };
    const detail = `${samples} samples • ${durationLabel(span)} span • ${method}${exactLabel}`;
    if (method.includes("active-session")) {
      if (samples >= 8 && span >= 30 * 60 * 1000) return { level: "high", label: "High confidence", detail };
      if (samples >= 4 && span >= XP_MIN_RATE_WINDOW_MS) return { level: "medium", label: "Medium confidence", detail };
      return { level: "low", label: "Low confidence", detail };
    }
    const staleThreshold = Number.isFinite(trend.windowMs) ? Math.min(24 * 60 * 60 * 1000, trend.windowMs / 2) : 24 * 60 * 60 * 1000;
    if (age > staleThreshold) return { level: "low", label: "Stale", detail: `${detail} • last seen ${ageLabel(trend.latest.at)} ago` };
    const coverage = Number.isFinite(trend.coverage) ? trend.coverage : 0;
    if (coverage >= 0.75 && samples >= 5 && !trend.delta?.estimated) return { level: "high", label: "High confidence", detail };
    if (coverage >= 0.35 && samples >= 3) return { level: "medium", label: "Medium confidence", detail };
    return { level: "low", label: "Low confidence", detail };
  };

  const filteredPerfectSystems = () => {
    const query = String(state.nodeFinder.query || "").trim().toLowerCase();
    const resource = state.nodeFinder.resource || "all";
    return nodeRows().filter((system) => (resource === "all" || system.nodes.some((node) => node.type.toLowerCase() === resource))
      && (!query || system.name.toLowerCase().includes(query) || `${system.x},${system.y}`.includes(query)
        || system.nodes.some((node) => `${node.type} ${node.body}`.toLowerCase().includes(query))));
  };

  const routeCoordinateKey = (row) => `${Number(row?.z ?? 1)}:${Number(row?.x)},${Number(row?.y)}`;
  const coordinateDistance = (from, to) => Number(from?.z ?? 1) === Number(to?.z ?? 1)
    ? Math.hypot(Number(to.x) - Number(from.x), Number(to.y) - Number(from.y))
    : Infinity;
  const galaxyDistance = (from, to) => coordinateDistance(from, to) * 10;

  const liveSquadStations = () => {
    const stations = getPinia()?._s?.get("squadronStore")?.spaceStations;
    if (!Array.isArray(stations)) return [];
    return stations.map((station) => ({
      key: routeCoordinateKey({ x: station?.system_parked?.coordinate_x, y: station?.system_parked?.coordinate_y, z: station?.system_parked?.coordinate_z }),
      id: String(station?._id || ""),
      systemId: String(station?.system_parked?._id || ""),
      name: String(station?.name || station?.system_parked?.name || "Squadron station"),
      kind: "station",
      x: finite(station?.system_parked?.coordinate_x),
      y: finite(station?.system_parked?.coordinate_y),
      z: finite(station?.system_parked?.coordinate_z ?? 1) ?? 1,
      portalLevel: Math.max(0, finite(station?.portal) || 0),
      portalEnabled: station?.space_portal === true,
      portalKnown: typeof station?.space_portal === "boolean",
      source: "live client",
    })).filter((station) => station.x !== null && station.y !== null);
  };

  const squadStationRows = () => {
    const squadron = String(state.profiles[state.selfProfileKey]?.squadron || "").toLowerCase();
    const official = state.officialApi.stations.filter((station) => !squadron || !station.squadron || station.squadron.toLowerCase() === squadron).map((station) => ({
      key: routeCoordinateKey(station),
      id: String(station.id || ""),
      name: String(station.name || "Squadron station"),
      kind: "station",
      x: finite(station.x),
      y: finite(station.y),
      z: finite(station.z ?? 1) ?? 1,
      portalLevel: 0,
      portalEnabled: false,
      portalKnown: false,
      source: "official station directory",
      squadron: String(station.squadron || ""),
    })).filter((station) => station.x !== null && station.y !== null);
    const rows = new Map(official.map((station) => [station.key, station]));
    for (const station of liveSquadStations()) rows.set(station.key, { ...(rows.get(station.key) || {}), ...station });
    return [...rows.values()].sort((a, b) => a.name.localeCompare(b.name));
  };

  const routeGameContext = () => {
    const pinia = getPinia();
    const explore = pinia?._s?.get("ExploreStore");
    const ship = pinia?._s?.get("ShipStore");
    const catalysts = pinia?._s?.get("CatalystStore");
    const boosts = pinia?._s?.get("GlobalBoostsStore");
    const pets = pinia?._s?.get("PetsStore");
    const currency = pinia?._s?.get("CurrencyStore");
    const liveTankCapacity = finite(explore?.total_fuel);
    const slots = ["weapon_slot", "shield_slot", "laser_slot", "probes_slot", "engine_slot", "sensors_slot"].map((key) => ship?.[key]).filter(Boolean);
    let catalystBonuses = {};
    try {
      catalystBonuses = catalysts?.getEquippedBonusesForShip?.(slots, "exploring") || {};
    } catch {}
    const cooldownGearBonus = [ship?.engine_slot, ship?.sensors_slot].reduce((total, item) => {
      const bonuses = Array.isArray(item?.bonuses) ? item.bonuses : [];
      return total + bonuses.reduce((sum, bonus) => sum + (bonus === "Engine cooldown reduction" ? bonuses.length === 1 ? 10 : 5 : 0), 0);
    }, 0);
    const korin = Array.isArray(pets?.pets) ? pets.pets.find((pet) => pet?.name === "Korin") : null;
    const korinEquipped = korin && Array.isArray(pets?.petSlots) && pets.petSlots.some((slot) => String(slot?.pet || "") === String(korin._id || ""));
    const cooldownGlobal = finite(boosts?.GlobalBoostTier?.find((boost) => boost?.boost === "Cooldown")?.bonus) || 0;
    return {
      current: currentCoordinates(),
      currentFuel: finite(explore?.current_fuel),
      totalFuel: liveTankCapacity !== null && liveTankCapacity > 0 ? liveTankCapacity : null,
      tankSource: liveTankCapacity !== null && liveTankCapacity > 0 ? "live game client" : "unavailable",
      credits: finite(currency?.credits),
      engineWait: Math.max(0, (finite(explore?.engineCooldown) || 0) - Date.now()),
      fuelEfficiency: Math.min(5, Math.max(0, finite(catalystBonuses?.fuel_efficiency) || 0)),
      cooldownReduction: Math.max(0, (finite(ship?.engine_slot?.value) || 0) / 100 + cooldownGearBonus + cooldownGlobal + (korinEquipped ? finite(korin?.level) || 0 : 0)),
      stations: squadStationRows(),
    };
  };

  const portalCreditCost = (station) => Math.floor(50000000 * 0.95 ** Math.max(0, finite(station?.portalLevel) || 0));
  const isStarterCoordinate = (row) => STARTER_STATIONS.some((station) => routeCoordinateKey(station) === routeCoordinateKey(row));
  const isStationCoordinate = (row, context) => context.stations.some((station) => station.source === "live client" && routeCoordinateKey(station) === routeCoordinateKey(row));
  const travelCooldownEstimate = (from, to, context) => {
    if (isStarterCoordinate(to) || isStationCoordinate(from, context) && isStationCoordinate(to, context)) return 0;
    const distance = galaxyDistance(from, to);
    return Number.isFinite(distance) ? Math.max(Math.floor(600000 * (1 + distance / 100) * (1 - context.cooldownReduction / 100)), 300000) : Infinity;
  };

  const directRouteLeg = (from, to, context) => {
    const distance = galaxyDistance(from, to);
    return {
      distance,
      segments: Number.isFinite(distance) ? [{ type: "travel", from, to, distance, fuel: distance * (1 - context.fuelEfficiency / 100) }] : [],
      portalCredits: 0,
    };
  };

  const jumpOffsetCache = new Map();
  const jumpPathCache = new Map();
  const legalJumpOffsets = (maxCoordinateDistance) => {
    const key = maxCoordinateDistance.toFixed(6);
    if (jumpOffsetCache.has(key)) return jumpOffsetCache.get(key);
    const limit = Math.floor(maxCoordinateDistance + 0.000001);
    const offsets = [];
    for (let x = -limit; x <= limit; x++) for (let y = -limit; y <= limit; y++) {
      if (x === 0 && y === 0) continue;
      const distance = Math.hypot(x, y);
      if (distance <= maxCoordinateDistance + 0.000001) offsets.push({ x, y, distance, distanceSquared: x * x + y * y });
    }
    offsets.sort((a, b) => b.distanceSquared - a.distanceSquared);
    jumpOffsetCache.set(key, offsets);
    return offsets;
  };

  const splitTravelSegment = (segment, context) => {
    const tankCapacity = finite(context.totalFuel);
    const fuelFactor = Math.max(0.01, 1 - context.fuelEfficiency / 100);
    const maxJumpDistance = tankCapacity !== null && tankCapacity > 0 ? tankCapacity / fuelFactor : null;
    if (!Number.isFinite(segment.distance) || segment.distance <= 0.000001) return [];
    if (!maxJumpDistance) return [{ ...segment, type: "jump", jump: 1, jumpCount: 1, legal: null }];
    const maxCoordinateDistance = maxJumpDistance / 10;
    const pathKey = [segment.from.x, segment.from.y, segment.from.z ?? 1, segment.to.x, segment.to.y, segment.to.z ?? 1, tankCapacity.toFixed(6), fuelFactor.toFixed(8)].join(":");
    let coordinates = jumpPathCache.get(pathKey);
    if (!coordinates) {
      const offsets = legalJumpOffsets(maxCoordinateDistance);
      const candidates = [];
      const sourceX = Number(segment.from.x);
      const sourceY = Number(segment.from.y);
      const targetX = Number(segment.to.x);
      const targetY = Number(segment.to.y);
      const deltaX = targetX - sourceX;
      const deltaY = targetY - sourceY;
      let balancedJumpCount = Math.max(1, Math.ceil(Math.hypot(deltaX, deltaY) / maxCoordinateDistance - 0.0000001));
      for (let attempt = 0; attempt < 1000; attempt++, balancedJumpCount++) {
        const balanced = [];
        let previous = { x: sourceX, y: sourceY, z: Number(segment.from.z ?? 1) };
        let valid = true;
        for (let index = 1; index <= balancedJumpCount; index++) {
          const point = index === balancedJumpCount
            ? { x: targetX, y: targetY, z: Number(segment.to.z ?? 1) }
            : { x: Math.round(sourceX + deltaX * index / balancedJumpCount), y: Math.round(sourceY + deltaY * index / balancedJumpCount), z: Number(segment.from.z ?? 1) };
          if (point.x === previous.x && point.y === previous.y) continue;
          if (Math.hypot(point.x - previous.x, point.y - previous.y) > maxCoordinateDistance + 0.000001) {
            valid = false;
            break;
          }
          balanced.push(point);
        ...
        if (valid && balanced.length && balanced.at(-1).x === targetX && balanced.at(-1).y === targetY) {
      ...
      for (let safety = 0; safety < 10000; safety++) {
        ...
        const remainingY = targetY - y;
        const remainingSquared = remainingX * remainingX + remainingY * remainingY;
        if (Math.sqrt(remainingSquared) <= maxCoordinateDistance + 0.000001) {
          progress.push({ x: targetX, y: targetY, z: Number(segment.to.z ?? 1) });
          break;
        }
        let best = null;
        let bestRemainingSquared = Infinity;
        for (const offset of offsets) {
          const nextRemainingX = remainingX - offset.x;
          const nextRemainingY = remainingY - offset.y;
          const nextRemainingSquared = nextRemainingX * nextRemainingX + nextRemainingY * nextRemainingY;
          if (nextRemainingSquared >= remainingSquared - 0.000001) continue;
          if (nextRemainingSquared < bestRemainingSquared - 0.000001 || Math.abs(nextRemainingSquared - bestRemainingSquared) <= 0.000001 && offset.distanceSquared > (best?.distanceSquared || 0)) {
            best = offset;
            bestRemainingSquared = nextRemainingSquared;
          }
        }
        if (!best) break;
        x += best.x;
        y += best.y;
        progress.push({ x, y, z: Number(segment.from.z ?? 1) });
      }
      if (progress.length && progress.at(-1).x === targetX && progress.at(-1).y === targetY) candidates.push(progress);
      const candidateDistance = (path) => {
        let total = 0;
        let previous = { x: sourceX, y: sourceY };
      ...
      coordinates = candidates.sort((a, b) => a.length - b.length || candidateDistance(a) - candidateDistance(b))[0] || [];
      if (coordinates.length && coordinates.at(-1).x === Number(segment.to.x) && coordinates.at(-1).y === Number(segment.to.y)) {
        jumpPathCache.set(pathKey, coordinates);
        if (jumpPathCache.size > 300) jumpPathCache.delete(jumpPathCache.keys().next().value);
      }
    }
    if (!coordinates?.length || coordinates.at(-1).x !== Number(segment.to.x) || coordinates.at(-1).y !== Number(segment.to.y)) return [{ ...segment, type: "jump", jump: 1, jumpCount: 1, legal: false }];
    const points = [segment.from, ...coordinates.map((coordinate, index) => index === coordinates.length - 1 ? segment.to : { ...coordinate, name: `Waypoint ${index + 1}`, kind: "waypoint" })];
    return points.slice(1).map((to, index) => {
      const from = points[index];
      const distance = galaxyDistance(from, to);
      const fuel = distance * fuelFactor;
      return { type: "jump", from, to, distance, fuel, jump: index + 1, jumpCount: points.length - 1, legal: fuel <= tankCapacity + 0.000001 };
    });
  };

  const shortestRouteLeg = (from, to, context) => {
    let best = directRouteLeg(from, to, context);
    if (!state.routePlanner.usePortals || !Number.isFinite(best.distance)) return best;
    const portals = context.stations.filter((station) => station.portalKnown && station.portalEnabled && Number(station.z) === Number(from.z) && Number(station.z) === Number(to.z));
    for (const entry of portals) for (const exit of portals) {
      if (entry.key === exit.key) continue;
      const firstDistance = galaxyDistance(from, entry);
      const lastDistance = galaxyDistance(exit, to);
      const distance = firstDistance + lastDistance;
      if (distance + 0.000001 >= best.distance) continue;
      const segments = [];
      if (firstDistance > 0.000001) segments.push({ type: "travel", from, to: entry, distance: firstDistance, fuel: firstDistance * (1 - context.fuelEfficiency / 100) });
      segments.push({ type: "teleport", from: entry, to: exit, credits: portalCreditCost(entry) });
      if (lastDistance > 0.000001) segments.push({ type: "travel", from: exit, to, distance: lastDistance, fuel: lastDistance * (1 - context.fuelEfficiency / 100) });
      best = { distance, segments, portalCredits: portalCreditCost(entry) };
    }
    return best;
  };

  const plannedRoute = () => {
    const context = routeGameContext();
    const origin = context.current;
    const destinations = state.routePlanner.destinations.filter((row) => Number(row.z) === Number(origin.z)).slice(0, ROUTE_MAX_DESTINATIONS);
    if (origin.x === null || origin.y === null || !destinations.length) return { context, origin, destinations, order: [], legs: [], distance: 0, fuel: 0, portalCredits: 0, arrivalWait: context.engineWait, readyWait: context.engineWait, jumpCount: 0, refillCount: 0, fuelRemaining: context.currentFuel, maxJumpDistance: context.totalFuel ? context.totalFuel / Math.max(0.01, 1 - context.fuelEfficiency / 100) : null, routeValid: context.totalFuel !== null };
    const count = destinations.length;
    const allMask = (1 << count) - 1;
    const best = new Map();
    for (let index = 0; index < count; index++) {
      const leg = shortestRouteLeg(origin, destinations[index], context);
      best.set(`${1 << index}:${index}`, { distance: leg.distance, order: [index] });
    }
    for (let mask = 1; mask <= allMask; mask++) for (let last = 0; last < count; last++) {
      const current = best.get(`${mask}:${last}`);
      if (!current) continue;
      for (let next = 0; next < count; next++) {
        if (mask & (1 << next)) continue;
        const leg = shortestRouteLeg(destinations[last], destinations[next], context);
        const nextMask = mask | (1 << next);
        const key = `${nextMask}:${next}`;
        const distance = current.distance + leg.distance;
        if (!best.has(key) || distance < best.get(key).distance) best.set(key, { distance, order: [...current.order, next] });
      }
    }
    const winner = [...best.entries()].filter(([key]) => key.startsWith(`${allMask}:`)).map(([, value]) => value).sort((a, b) => a.distance - b.distance)[0];
    const order = (winner?.order || []).map((index) => destinations[index]);
    const legs = [];
    let cursor = origin;
    for (const destination of order) {
      const leg = shortestRouteLeg(cursor, destination, context);
      const steps = leg.segments.flatMap((segment) => segment.type === "travel" ? splitTravelSegment(segment, context) : [segment]);
      legs.push({ ...leg, destination, steps });
      cursor = destination;
    }
    const segments = legs.flatMap((leg) => leg.steps);
    let arrivalWait = 0;
    let pendingCooldown = context.engineWait;
    const tankCapacity = finite(context.totalFuel);
    let fuelRemaining = tankCapacity !== null && tankCapacity > 0
      ? Math.min(tankCapacity, Math.max(0, context.currentFuel === null ? tankCapacity : context.currentFuel))
      : null;
    let refillCount = tankCapacity !== null && tankCapacity > 0 ? 0 : null;
    let jumpCount = 0;
    let routeValid = tankCapacity !== null && tankCapacity > 0;
    for (const segment of segments) {
      ...
      jumpCount++;
      arrivalWait += pendingCooldown;
      pendingCooldown = travelCooldownEstimate(segment.from, segment.to, context);
      segment.cooldown = pendingCooldown;
      segment.waitBefore = arrivalWait;
      if (segment.legal === false || tankCapacity === null || tankCapacity <= 0) routeValid = false;
      if (fuelRemaining !== null) {
        if (segment.fuel > fuelRemaining + 0.000001) {
          refillCount++;
          fuelRemaining = tankCapacity;
          segment.refillBefore = true;
        }
        fuelRemaining = Math.max(0, fuelRemaining - segment.fuel);
      }
    }
    return {
      context,
      ...
      order,
      legs,
      distance: segments.reduce((sum, segment) => sum + (segment.type === "jump" ? segment.distance : 0), 0),
      fuel: segments.reduce((sum, segment) => sum + (segment.type === "jump" ? segment.fuel : 0), 0),
      portalCredits: segments.reduce((sum, segment) => sum + (segment.type === "teleport" ? segment.credits : 0), 0),
      arrivalWait,
      readyWait: arrivalWait + pendingCooldown,
      jumpCount,
      refillCount,
      fuelRemaining,
      maxJumpDistance: tankCapacity ? tankCapacity / Math.max(0.01, 1 - context.fuelEfficiency / 100) : null,
      routeValid,
    };
  };

  const routeDestinationButton = (row) => {
    const key = routeCoordinateKey(row);
    const selected = state.routePlanner.destinations.some((destination) => destination.key === key);
    return `<button data-action="plan-route" data-route-key="${esc(key)}" data-route-name="${esc(row.name || row.systemName || "Destination")}" data-route-kind="${esc(row.kind || "destination")}" data-route-x="${Number(row.x)}" data-route-y="${Number(row.y)}" data-route-z="${Number(row.z ?? 1)}" ${selected ? "disabled" : ""}>${selected ? "Planned ✓" : "Plan route"}</button>`;
  };

  const routeLegDescription = (leg) => {
    const jumps = leg.steps.filter((segment) => segment.type === "jump");
    const portal = leg.steps.find((segment) => segment.type === "teleport");
    return `${jumps.length} jump${jumps.length === 1 ? "" : "s"} • ${jumps.reduce((sum, jump) => sum + jump.distance, 0).toFixed(1)} ly${portal ? ` • ${portal.from.name} → ${portal.to.name} portal` : ""}`;
  };

  const routeLegItinerary = (leg) => {
    let jump = 0;
    return leg.steps.map((segment) => {
      if (segment.type === "teleport") return `PORTAL  ${segment.from.name} [${segment.from.x}, ${segment.from.y}, ${segment.from.z}] → ${segment.to.name} [${segment.to.x}, ${segment.to.y}, ${segment.to.z}] • ${compactNumber(segment.credits)} credits • resets cooldown`;
      jump++;
      const refill = segment.refillBefore ? " • REFILL BEFORE JUMP" : "";
      return `J${String(jump).padStart(3, "0")}  [${segment.from.x}, ${segment.from.y}, ${segment.from.z}] → [${segment.to.x}, ${segment.to.y}, ${segment.to.z}] • ${segment.distance.toFixed(1)} ly • ${segment.fuel.toFixed(1)} fuel • ${cooldownTimeLabel(segment.cooldown)} cooldown${refill}`;
    }).join("\n");
  };
  ...
  let perfectMapRevision = -1;
  let perfectMapSystems = null;
  let perfectMapCache = new Map();
  const perfectSystemsByCoordinate = () => {
    if (perfectMapRevision === nodeDataRevision && perfectMapSystems === state.systems) return perfectMapCache;
    const merged = new Map();
    for (const system of publicAtlas.systems) {
      const key = `${system.z}:${system.x},${system.y}`;
      if (Number(state.nodeObservations[key] || 0) > publicAtlas.fetchedAt && !state.systems[key]) continue;
      merged.set(key, system);
    }
    for (const [key, system] of Object.entries(state.systems)) {
      const verifiedAt = Number(state.nodeObservations[key] || system?.verifiedAt || 0);
      if (!verifiedAt) continue;
      const nodes = (Array.isArray(system?.nodes) ? system.nodes : []).filter((node) => Number(node?.quality) === 100);
      if (!nodes.length) continue;
      const existing = merged.get(key);
      if (!existing || verifiedAt >= publicAtlas.fetchedAt) merged.set(key, { ...system, nodes, source: "Personal observation" });
    }
    perfectMapRevision = nodeDataRevision;
    perfectMapSystems = state.systems;
    perfectMapCache = merged;
    return merged;
  };
  let nearestRowsKey = "";
  let nearestRowsMap = null;
  let nearestRowsCache = [];
  const nodeRows = () => {
    const current = currentCoordinates();
    const currentX = current.x;
    const currentY = current.y;
    const currentZ = current.z;
    const systems = perfectSystemsByCoordinate();
    const key = `${currentX}:${currentY}:${currentZ}`;
    if (key === nearestRowsKey && systems === nearestRowsMap) return nearestRowsCache;
    nearestRowsKey = key;
    nearestRowsMap = systems;
    nearestRowsCache = [...systems.values()].map((system) => ({
      ...system,
    ...
    return nearestRowsCache;
  };

  const renderPerfectFinder = (rows) => {
    const current = currentCoordinates();
    const hasPosition = current.x !== null && current.y !== null;
    const nearest = hasPosition ? rows.filter((row) => row.distanceFromCurrent !== null) : rows;
    const status = publicAtlas.hasSnapshot
      ? `${compactNumber(publicAtlas.totalSystems)} public systems indexed • ${publicAtlas.systems.length} perfect systems • checked ${ageLabel(publicAtlas.checkedAt || publicAtlas.fetchedAt)} ago`
      : publicAtlas.refreshing ? "Building the public perfect-node index in the background…"
        : publicAtlas.message || "Add your official API key in Sync to search the public galaxy.";
    return `<section data-section="perfect-finder"><h3>Nearest perfect nodes</h3>
      <div class="so-tool-controls" data-node-filters>
        <label>Resource<select data-action="node-resource">${[["all", "All resources"], ["rocky", "Rocky"], ["icy", "Icy"], ["gas", "Gas"], ["crystal", "Crystal"]].map(([value, label]) => `<option value="${value}" ${state.nodeFinder.resource === value ? "selected" : ""}>${label}</option>`).join("")}</select></label>
        <label>System or body<input data-action="node-query" value="${esc(state.nodeFinder.query)}" placeholder="Name, coordinates, or planet"></label>
      </div>
      <p class="so-note">${esc(status)}${publicAtlas.hasSnapshot && publicAtlas.refreshing ? " • refreshing in the background" : ""}${publicAtlas.stale ? " • saved snapshot; refresh delayed" : ""}</p>
      ${publicAtlas.hasSnapshot && publicAtlas.message ? `<p class="so-note">${esc(publicAtlas.message)}</p>` : ""}
      <p class="so-note">${hasPosition ? `From [${current.x}, ${current.y}, ${current.z}] • nearest by straight-line distance • updates after jumps and teleports` : "Waiting for your current game location; distances are unavailable."}</p>
      <div class="so-list">${nearest.slice(0, 10).map((system) => `
        <article class="so-row so-perfect-result">
          <div class="so-row-main"><strong>${esc(system.name)}</strong><span>[${system.x}, ${system.y}, ${system.z}]${system.distanceFromCurrent !== null ? ` • ${system.distanceFromCurrent.toFixed(1)} units away` : ""} • ${esc(system.source)}</span></div>
          <div class="so-chips">${system.nodes.filter((node) => state.nodeFinder.resource === "all" || node.type.toLowerCase() === state.nodeFinder.resource).map((node) => `<span title="${esc(node.body)}">${esc(node.type)} 100%</span>`).join("")}</div>
          <div class="so-inline-actions">${routeDestinationButton({ ...system, kind: "perfect" })}</div>
        </article>`).join("") || `<div class="so-empty">${hasPosition && current.z !== 1 && publicAtlas.hasSnapshot ? "The public API covers the main galaxy (Z 1). No personal perfect-node match is known on your current layer." : "No exact 100% matches are available for this filter yet."}</div>`}</div>
      ${nearest.length > 10 ? `<p class="so-note">Showing the nearest 10 of ${nearest.length} matching systems.</p>` : ""}
      <p class="so-note">Public map discoveries do not increase your personal systems-seen count. Select Plan route to calculate fuel, jump cooldowns, and available portal shortcuts.</p>
    </section>`;
  };
  ...
  const renderNodes = () => {
    ...
    const stationRows = plan.context.stations;
    const portalCount = stationRows.filter((station) => station.portalKnown && station.portalEnabled).length;
    const routeTimeLabel = (value) => value <= 0 ? "Ready now" : cooldownTimeLabel(value);
    const creditsShortfall = plan.context.credits === null ? null : Math.max(0, plan.portalCredits - plan.context.credits);
    return `
      ...
      ${renderPerfectFinder(rows)}
      <section><div class="so-section-heading"><h3>Shortest route planner</h3>${state.routePlanner.destinations.length ? `<button data-action="clear-route">Clear route</button>` : ""}</div>
        <p class="so-note">Select up to ${ROUTE_MAX_DESTINATIONS} destinations. The planner finds the exact shortest visit order, compares direct flight with every currently loaded squad-portal pair, then minimizes legal fuel-limited jump count and flown distance. It only displays a plan—it never travels or teleports.</p>
        <label class="so-route-toggle"><input type="checkbox" data-action="route-use-portals" ${state.routePlanner.usePortals ? "checked" : ""}><span><strong>Use squad portal shortcuts</strong><small>${portalCount} active portal${portalCount === 1 ? "" : "s"} verified in the live squad-station data</small></span></label>
        <label class="so-route-toggle"><input type="checkbox" data-action="route-show-on-maps" ${state.routePlanner.showOnMaps ? "checked" : ""}><span><strong>Show planned route on maps</strong><small>Jump lines and labels on the galaxy map, plus an overview path on its corner minimap</small></span></label>
        ${plan.order.length ? `
          <div class="so-route-summary">
            <div><span>Flown distance</span><strong>${plan.distance.toFixed(1)} ly</strong></div>
            <div><span>Flight jumps</span><strong>${plan.jumpCount}</strong><small>${plan.maxJumpDistance === null ? "jump range unavailable" : `${plan.maxJumpDistance.toFixed(1)} ly maximum`}</small></div>
            <div><span>Live fuel tank</span><strong>${plan.context.totalFuel === null ? "Unavailable" : plan.context.totalFuel.toFixed(1)}</strong><small>${plan.context.totalFuel === null ? "open the game map to load it" : `${plan.context.currentFuel === null ? "current fuel unavailable" : `${plan.context.currentFuel.toFixed(1)} currently`} • verified from game`}</small></div>
            <div><span>Fuel consumed</span><strong>${plan.fuel.toFixed(1)}</strong><small>${plan.fuelRemaining === null ? "ending fuel unavailable" : `${plan.fuelRemaining.toFixed(1)} remaining`}</small></div>
            <div><span>Tank refills</span><strong>${plan.refillCount === null ? "Unavailable" : plan.refillCount}</strong><small>estimated full-tank refills</small></div>
            <div><span>Arrival wait</span><strong>${routeTimeLabel(plan.arrivalWait)}</strong></div>
            <div><span>Engine ready</span><strong>${routeTimeLabel(plan.readyWait)}</strong><small>after final arrival</small></div>
            <div><span>Portal cost</span><strong>${compactNumber(plan.portalCredits)}</strong><small>credits</small></div>
          </div>
          ${!plan.routeValid ? `<div class="so-route-warning">The live fuel-tank capacity is not available yet, so legal jump splitting cannot be verified. Open the galaxy map and wait for the fuel meter to load.</div>` : ""}
          ${plan.refillCount > 0 ? `<div class="so-route-warning">This route needs ${plan.refillCount} estimated full-tank refill${plan.refillCount === 1 ? "" : "s"}, based on the live ${plan.context.totalFuel.toFixed(1)}-fuel capacity${plan.context.currentFuel === null ? " and an assumed full starting tank" : ` and ${plan.context.currentFuel.toFixed(1)} fuel currently loaded`}. Refilling remains manual.</div>` : ""}
          ${creditsShortfall > 0 ? `<div class="so-route-warning">Portal legs need ${compactNumber(creditsShortfall)} more credits than the current balance.</div>` : ""}
          <div class="so-route-list">${plan.legs.map((leg, index) => `
            <article class="so-route-row"><b>${index + 1}</b><div><strong>${esc(leg.destination.name)}</strong><span>[${leg.destination.x}, ${leg.destination.y}, ${leg.destination.z}] • ${esc(routeLegDescription(leg))}</span></div><div class="so-route-actions"><button data-action="toggle-route-leg" data-route-key="${esc(leg.destination.key)}">${state.routePlanner.expandedLeg === leg.destination.key ? "Hide jumps" : `Show ${leg.steps.filter((step) => step.type === "jump").length} jumps`}</button><button data-action="remove-route-stop" data-route-key="${esc(leg.destination.key)}">Remove</button></div>${state.routePlanner.expandedLeg === leg.destination.key ? `<pre class="so-route-itinerary">${esc(routeLegItinerary(leg))}</pre>` : ""}</article>
          `).join("")}</div>
          <p class="so-route-footnote">Every flight is split across integer systems using the fewest legal jumps found, then the shortest flown path at that jump count. No jump exceeds the live ${plan.context.totalFuel === null ? "unavailable" : plan.context.totalFuel.toFixed(1)}-fuel tank. Range includes your capped ${plan.context.fuelEfficiency.toFixed(1)}% fuel-saving bonus; time uses the live ${plan.context.cooldownReduction.toFixed(1)}% cooldown reduction and current engine timer. Portal costs use the departure station’s live portal level.</p>
        ` : `<div class="so-empty">Choose <strong>Plan route</strong> on a perfect node, event, squad station, or starter system.</div>`}
      </section>
      <section><h3>Galaxy event radar</h3>
        ...
        <div class="so-event-list">${events.length ? events.slice(0, 40).map((event) => `
          <article class="so-event ${esc(event.kind)}">
            ...
            ${event.x !== null && event.y !== null ? routeDestinationButton({ ...event, name: event.systemName || event.name, kind: event.kind }) : ""}
      ...
      ${stationRows.length ? `<section><h3>Squadron stations</h3><div class="so-list">${stationRows.slice(0, 30).map((station) => `
        <article class="so-row"><div class="so-row-main"><strong>${esc(station.name)}</strong><span>[${station.x}, ${station.y}, ${station.z}]</span></div><div class="so-chips muted"><span>${station.portalKnown ? station.portalEnabled ? `Portal active • ${compactNumber(portalCreditCost(station))} credits out` : "Portal inactive" : "Portal status loads from the live game"}</span></div><div class="so-inline-actions">${routeDestinationButton(station)}</div></article>
      `).join("")}</div></section>` : ""}
      <section><h3>Starter systems</h3><p class="so-note">Starter arrivals clear the engine cooldown, so the time estimate accounts for them when selected. The current game client does not expose starter-to-starter teleport; only verified squad portals are used as teleport shortcuts.</p><div class="so-list">${STARTER_STATIONS.map((station) => `
        <article class="so-row"><div class="so-row-main"><strong>${esc(station.name)}</strong><span>[${station.x}, ${station.y}, ${station.z}]</span></div><div class="so-chips muted"><span>${esc(station.star)}</span><span>Cooldown-free arrival</span></div><div class="so-inline-actions">${routeDestinationButton({ ...station, kind: "starter" })}</div></article>
      `).join("")}</div></section>`;
  };
  ...
  const renderRoster = () => {
    ...
    const stations = squadStationRows();
    return `
      ...
      <section><h3>Squadron stations</h3>${stations.length ? `<div class="so-list">${stations.map((station) => `<article class="so-row"><div class="so-row-main"><strong>${esc(station.name)}</strong><span>[${station.x}, ${station.y}, ${station.z}]</span></div><div class="so-inline-actions">${routeDestinationButton(station)}</div></article>`).join("")}</div>` : `<div class="so-empty">No station for the loaded squad is present in the current official snapshot.</div>`}</section>
      ...
      <section><h3>Coordination notes</h3><p class="so-note">Use Plan route on a station to add it to the exact shortest-order calculation in Galaxy Intel. Route plans stay informational and local to this device.</p></section>`;
  };
  ...
  let nativeEngineScheduleSignature = "";
  let nativeEngineScheduled = false;
  const nativeOperationSchedules = new Map();

  const nativeAlertsAvailable = () => {
    const version = nativeBridgeConfig()?.launcherVersion.split(".").map((part) => Number(part) || 0) || [];
    return (version[0] || 0) > 2 || ((version[0] || 0) === 2 && (version[1] || 0) >= 1);
  };

  const gameHasFocus = () => document.visibilityState !== "hidden" && document.hasFocus();

  const nativeAlertId = (prefix, value) => {
    let hash = 2166136261;
  ...
  const scheduleNativeAlert = async ({ id, title, body, dueAt, expiresAt, backgroundOnly = true }) => {
    if (!nativeAlertsAvailable()) return false;
    try {
      const result = await nativeBridgeJson("/alert/schedule", {
        method: "POST",
        timeoutMs: 5000,
        body: {
          id,
          ...
          dueAt,
          expiresAt,
          desktop: state.engineAlerts.desktop,
          sound: state.engineAlerts.sound && state.engineAlerts.volume > 0,
          attention: state.engineAlerts.flash,
          backgroundOnly,
          volume: state.engineAlerts.volume,
        },
      });
      return result.scheduled === true;
    } catch (error) {
      console.warn("[Starwatch] Native alert scheduling unavailable", error);
      return false;
    }
  };

  const cancelNativeAlert = async (id) => {
    if (!nativeAlertsAvailable()) return;
    try { await nativeBridgeJson("/alert/cancel", { method: "POST", body: { id }, timeoutMs: 5000 }); }
    catch { }
  };

  const resetNativeOperationAlerts = async () => {
    if (!nativeAlertsAvailable()) return;
    try { await nativeBridgeJson("/alert/cancel-prefix", { method: "POST", body: { prefix: "operation-" }, timeoutMs: 5000 }); }
    catch { }
  };

  const syncNativeEngineAlert = () => {
    const cooldown = state.engineCooldown;
    if (cooldown.phase !== "armed" || !cooldown.readyAt || !nativeAlertsAvailable()) return;
    const signature = [
      Math.round(cooldown.readyAt / 1000),
      state.engineAlerts.desktop,
      state.engineAlerts.sound,
      state.engineAlerts.flash,
      Math.round(state.engineAlerts.volume * 100),
    ].join(":");
    if (signature === nativeEngineScheduleSignature) return;
    nativeEngineScheduleSignature = signature;
    nativeEngineScheduled = false;
    scheduleNativeAlert({
      id: "engine-ready",
      title: "Stellar Odyssey — Engine ready",
      body: "Engine cooldown is complete and travel is available again.",
      dueAt: cooldown.readyAt,
      expiresAt: cooldown.readyAt + 10 * 60 * 1000,
    }).then((scheduled) => {
      if (signature === nativeEngineScheduleSignature) nativeEngineScheduled = scheduled;
    });
  };
  ...
  const playEngineChime = () => {
    ...
    try {
      ...
      console.warn("[Starwatch] Engine alert audio unavailable", error);
    }
  };
  ...
  const showDesktopEngineNotification = () => {
    ...
    try {
      ...
      console.warn("[Starwatch] Desktop notification unavailable", error);
    }
  };

  const showDesktopIntelNotification = (title, detail) => {
    if (!state.engineAlerts.desktop || !("Notification" in window) || window.Notification.permission !== "granted") return;
  ...
  const triggerEngineReady = (test = false) => {
    ...
    const foreground = gameHasFocus();
    const nativeTest = test && nativeAlertsAvailable() && (state.engineAlerts.desktop || state.engineAlerts.sound || state.engineAlerts.flash);
    const rendererChannels = !nativeTest && (!nativeEngineScheduled || foreground);
    if (rendererChannels) playEngineChime();
    showEngineToast(test ? "Test notification — engine ready" : "Cooldown complete — engine ready");
    flashEngineReady();
    if (rendererChannels) showDesktopEngineNotification();
    if (nativeTest) {
      const dueAt = Date.now() + 100;
      scheduleNativeAlert({
        id: nativeAlertId("test", dueAt),
        title: "Stellar Odyssey — Test alert",
        body: "Background notifications from Wookie's Starwatch are working.",
        dueAt,
        expiresAt: dueAt + 2 * 60 * 1000,
  ...
  const notifyIntel = (title, detail, scheduleNative = true) => {
    const foreground = gameHasFocus();
    const native = scheduleNative && nativeAlertsAvailable() && (state.engineAlerts.desktop || state.engineAlerts.sound || state.engineAlerts.flash);
    if (!native || foreground) playEngineChime();
    showEngineToast(`${title} — ${detail}`, true);
    if (!native || foreground) showDesktopIntelNotification(title, detail);
    if (native) {
      const dueAt = Date.now() + 100;
      scheduleNativeAlert({
        id: nativeAlertId("intel", `${title}:${detail}:${Math.floor(dueAt / 10000)}`),
        title: `Stellar Odyssey — ${title}`,
        body: detail,
        dueAt,
        expiresAt: dueAt + 2 * 60 * 1000,
      }).then((scheduled) => {
        if (!scheduled && !foreground) {
          playEngineChime();
          showDesktopIntelNotification(title, detail);
        }
      });
    }
  };
  ...
  const checkOperationAlerts = () => {
    const now = Date.now();
    const activeNativeIds = new Set();
    for (const row of operationTimeline()) {
      const signature = `${row.key}:${row.dueAt}`;
      const nativeId = nativeAlertId("operation", row.key);
      const existingNative = nativeOperationSchedules.get(nativeId);
      if (state.engineAlerts.operations && row.dueAt > now && nativeAlertsAvailable()) {
        activeNativeIds.add(nativeId);
        const nativeSignature = [signature, state.engineAlerts.desktop, state.engineAlerts.sound, state.engineAlerts.flash, Math.round(state.engineAlerts.volume * 100)].join(":");
        if (nativeOperationSchedules.get(nativeId)?.signature !== nativeSignature) {
          const record = { signature: nativeSignature, scheduled: false };
          nativeOperationSchedules.set(nativeId, record);
          scheduleNativeAlert({
            id: nativeId,
            title: `Stellar Odyssey — ${row.kind} ready`,
            body: row.name,
            dueAt: row.dueAt,
            expiresAt: row.dueAt + 10 * 60 * 1000,
          }).then((scheduled) => {
            if (nativeOperationSchedules.get(nativeId) === record) record.scheduled = scheduled;
          });
        }
      } else if (state.engineAlerts.operations && existingNative && row.dueAt + 10 * 60 * 1000 > now) {
        activeNativeIds.add(nativeId);
      }
      ...
      if (!previous) operationAlertState.set(row.key, { signature, wasFuture: row.dueAt > now });
        ...
        const nativeHandled = nativeOperationSchedules.get(nativeId)?.scheduled === true;
        if (state.engineAlerts.operations) {
          if (!nativeHandled || gameHasFocus()) notifyIntel(`${row.kind} ready`, row.name, !nativeHandled);
          else showEngineToast(`${row.kind} ready — ${row.name}`, true);
        }
      }
    }
    for (const nativeId of [...nativeOperationSchedules.keys()]) {
      if (!activeNativeIds.has(nativeId)) {
        nativeOperationSchedules.delete(nativeId);
        cancelNativeAlert(nativeId);
      }
    }
  };
  ...
  const requestDesktopNotifications = async () => {
    if (nativeAlertsAvailable()) {
      showEngineToast("Windows background notifications enabled through the Starwatch launcher", true);
  ...
  const xpSamplesForWindow = (history, activity, windowName) => {
    const samples = (Array.isArray(history) ? history : [])
      .filter((sample) => sample.activities?.[activity] && finite(sample?.at) !== null)
      .sort((a, b) => a.at - b.at);
    const latest = samples.at(-1) || null;
    const windowMs = XP_WINDOWS[windowName] ?? XP_WINDOWS["24h"];
    if (!latest) return { latest, windowMs, samples: [], staleBeyondWindow: false, configurationChanged: false };
    const staleBeyondWindow = Number.isFinite(windowMs) && Date.now() - Number(latest.at) > windowMs;
    if (staleBeyondWindow) return { latest, windowMs, samples: [], staleBeyondWindow, configurationChanged: false };
    if (!Number.isFinite(windowMs)) {
      const segment = latestCompatibleXpSegment(samples);
      return { latest, windowMs, staleBeyondWindow, ...segment };
    }
    const cutoff = Date.now() - windowMs;
    let nearestIndex = 0;
    let nearestDistance = Infinity;
    for (let index = 0; index < samples.length - 1; index += 1) {
      const distance = Math.abs(Number(samples[index].at) - cutoff);
      if (distance < nearestDistance) {
        nearestDistance = distance;
        nearestIndex = index;
      }
    }
    const segment = latestCompatibleXpSegment(samples.slice(nearestIndex));
    return { latest, windowMs, staleBeyondWindow, ...segment };
  };

  const xpTrend = (history, activity, windowName) => {
    const selection = xpSamplesForWindow(history, activity, windowName);
    ...
    if (!latest || samples.length < 2) return {
      ...
      windowMs: selection.windowMs,
      staleBeyondWindow: selection.staleBeyondWindow,
      configurationChanged: selection.configurationChanged,
    };
    const pairs = [];
    for (let index = 1; index < samples.length; index += 1) {
      ...
      const delta = elapsed >= XP_SAMPLE_BUCKET_MS ? xpDelta(first, last, activity) : null;
      if (!delta || delta.gained < 0) continue;
      pairs.push({ elapsed, gained: delta.gained, rate: delta.gained / (elapsed / 3600000), estimated: delta.estimated, missingLevels: delta.missingLevels || 0 });
    }
    const positiveMedian = medianNumber(pairs.filter((pair) => pair.rate > 0).map((pair) => pair.rate));
    const accepted = positiveMedian > 0 ? pairs.filter((pair) => pair.rate <= positiveMedian * 6) : pairs;
    const elapsed = accepted.reduce((total, pair) => total + pair.elapsed, 0);
    const gained = accepted.reduce((total, pair) => total + pair.gained, 0);
    const outliers = pairs.length - accepted.length;
    const rate = accepted.length && elapsed >= XP_MIN_RATE_WINDOW_MS ? gained / (elapsed / 3600000) : null;
    return {
      latest,
      baseline: samples[0],
      elapsed,
      delta: accepted.length ? {
        gained,
        estimated: accepted.some((pair) => pair.estimated),
        confidence: accepted.some((pair) => pair.estimated) ? "low" : "high",
        missingLevels: accepted.reduce((total, pair) => total + pair.missingLevels, 0),
      } : null,
      rate,
      withheld: Boolean(pairs.length && !accepted.length),
      outliers,
      sampleCount: accepted.length + (accepted.length ? 1 : 0),
      windowMs: selection.windowMs,
      coverage: Number.isFinite(selection.windowMs) ? Math.min(1, elapsed / selection.windowMs) : 1,
      staleBeyondWindow: selection.staleBeyondWindow,
      configurationChanged: selection.configurationChanged,
      method: accepted.length > 1 ? "robust multi-snapshot trend" : "two-point observation",
    };
  };

  const xpActiveTrend = (history, activity, windowName) => {
    const selection = xpSamplesForWindow(history, activity, windowName);
    const rawSamples = selection.samples;
    const samples = rawSamples.filter((sample, index) => {
      if (!index) return true;
      const previous = rawSamples[index - 1].activities[activity];
      const current = sample.activities[activity];
      return Number(current?.[0]) !== Number(previous?.[0]) || Number(current?.[1]) !== Number(previous?.[1]);
    });
    const latest = samples.at(-1);
    if (!latest) return { activeRate: null, activeElapsed: 0, activeGained: 0, activeSampleCount: 0, activeOutliers: 0, activeLatest: selection.latest, activeWindowMs: selection.windowMs, activeConfigurationChanged: selection.configurationChanged };
    ...
    if (!pairs.length) return { activeRate: null, activeElapsed: 0, activeGained: 0, activeSampleCount: 0, activeOutliers: 0, activeLatest: latest, activeWindowMs: selection.windowMs, activeConfigurationChanged: selection.configurationChanged };
    const medianRate = (rows) => medianNumber(rows.map((pair) => pair.rate));
    const median = medianRate(pairs);
    ...
    return {
      activeRate: ready ? medianRate(accepted) : null,
      activeElapsed,
      activeGained,
      ...
      activeLatest: latest,
      activeWindowMs: selection.windowMs,
      activeCoverage: Number.isFinite(selection.windowMs) ? Math.min(1, activeElapsed / selection.windowMs) : 1,
      activeConfigurationChanged: selection.configurationChanged,
    };
  };
  ...
  const xpTrendForKey = (key, activity, windowName) => {
    ...
    const windowMs = XP_WINDOWS[windowName] ?? XP_WINDOWS["24h"];
    const staleBeyondWindow = Number.isFinite(windowMs) && Date.now() - Number(latest.at) > windowMs;
    const startContext = xpContextKey(start);
    const latestContext = xpContextKey(latest);
    const configurationChanged = Boolean(startContext && latestContext && startContext !== latestContext);
    const shared = {
      latest,
      baseline: start,
      elapsed,
      delta,
      rate: !staleBeyondWindow && !configurationChanged && delta && elapsed >= XP_MIN_RATE_WINDOW_MS ? delta.gained / (elapsed / 3600000) : null,
      sampleCount: 2,
      outliers: 0,
      withheld: false,
      windowMs,
      coverage: Number.isFinite(windowMs) ? Math.min(1, elapsed / windowMs) : 1,
      staleBeyondWindow,
      configurationChanged,
      method: "shared two-point observation",
    };
    const localReady = Number.isFinite(local.rate);
    const sharedReady = Number.isFinite(shared.rate);
    const selected = localReady && (Number(local.latest?.at || 0) >= Number(shared.latest?.at || 0) || !sharedReady || local.sampleCount >= 3)
      ? local
      : sharedReady
        ? shared
        : Number(local.latest?.at || 0) >= Number(shared.latest?.at || 0) ? local : shared;
  ...
  const xpLeaderboard = (keys, activity, windowName) => {
    const rows = keys.map((key) => ({
      ...
      // Do not use finite() for availability checks: Number(null) is 0 and would
      // incorrectly prevent a player without active data from using wall-clock fallback.
      const usesActive = Number.isFinite(row.trend.activeRate);
      const usesWall = !usesActive && Number.isFinite(row.trend.rate);
      return {
        ...row,
        rankingRate: usesActive ? row.trend.activeRate : usesWall ? row.trend.rate : null,
        rateSource: usesActive ? "active" : usesWall ? "wall" : null,
      };
    });
    const ranked = rows
      ...
      <section class="so-xp-leaderboard-card">
        ...
        <div class="so-xp-leaderboard-list">
          ${board.ranked.length ? board.ranked.map((row) => {
            const evidence = row.rateSource === "active"
              ? {
                ...row.trend,
                latest: row.trend.activeLatest || row.trend.latest,
                rate: row.trend.activeRate,
                elapsed: row.trend.activeElapsed,
                delta: { gained: row.trend.activeGained, estimated: false, missingLevels: 0 },
                sampleCount: row.trend.activeSampleCount,
                outliers: row.trend.activeOutliers,
                withheld: row.trend.activeWithheld,
                windowMs: row.trend.activeWindowMs,
                coverage: row.trend.activeCoverage,
                configurationChanged: row.trend.activeConfigurationChanged,
                method: "active-session median",
              }
              : row.trend;
            const confidence = confidenceForTrend(evidence, XP_MIN_RATE_WINDOW_MS);
            const sourceLabel = row.rateSource === "active" ? "active" : "wall";
            return `
            <div class="so-xp-leaderboard-row ${row.key === state.selfProfileKey ? "self" : ""}" title="${esc(confidence.detail)}">
              ...
              <strong>${row.rateSource === "wall" && row.trend.delta?.estimated ? "≈" : ""}${esc(xpRateNumber(row.rankingRate))}<small> ${sourceLabel} XP/h</small></strong>
            </div>`;
          }).join("") : `<div class="so-empty">No players have completed calibration for ${esc(activity)}.</div>`}
        </div>
        <div class="so-xp-leaderboard-foot">${board.ranked.length} ranked • ${board.ranked.filter((row) => row.rateSource === "active").length} active • ${board.ranked.filter((row) => row.rateSource === "wall").length} wall-clock fallback • ${board.calibrating.filter((row) => !row.trend.withheld).length} calibrating • ${board.calibrating.filter((row) => row.trend.withheld).length} withheld</div>
      </section>`;
  };
  ...
  const resourceTrend = (history, windowName) => {
    ...
    const windowMs = XP_WINDOWS[windowName] ?? XP_WINDOWS["24h"];
    const staleBeyondWindow = Number.isFinite(windowMs) && Date.now() - Number(latest.at) > windowMs;
    if (staleBeyondWindow) return { latest, rate: null, elapsed: 0, sampleCount: 0, windowMs, staleBeyondWindow };
    let nearestIndex = 0;
    if (Number.isFinite(windowMs)) {
      const cutoff = Date.now() - windowMs;
      let nearestDistance = Infinity;
    ...
    const measuredSamples = samples.slice(nearestIndex);
    const pairs = [];
    for (let index = 1; index < measuredSamples.length; index += 1) {
      const elapsed = Number(measuredSamples[index].at) - Number(measuredSamples[index - 1].at);
      const gained = finite(measuredSamples[index].value) - finite(measuredSamples[index - 1].value);
      if (elapsed >= XP_SAMPLE_BUCKET_MS && gained >= 0) pairs.push({ elapsed, gained, rate: gained / (elapsed / 3600000) });
    }
    const positiveMedian = medianNumber(pairs.filter((pair) => pair.rate > 0).map((pair) => pair.rate));
    const accepted = positiveMedian > 0 ? pairs.filter((pair) => pair.rate <= positiveMedian * 6) : pairs;
    const elapsed = accepted.reduce((total, pair) => total + pair.elapsed, 0);
    const gained = accepted.reduce((total, pair) => total + pair.gained, 0);
    const outliers = pairs.length - accepted.length;
    const eligibleRate = accepted.length && elapsed >= RESOURCE_MIN_RATE_WINDOW_MS ? gained / (elapsed / 3600000) : null;
    return {
      latest,
      baseline: measuredSamples[0],
      elapsed,
      gained: accepted.length ? gained : null,
      rate: eligibleRate,
      withheld: Boolean(pairs.length && !accepted.length),
      outliers,
      sampleCount: accepted.length + (accepted.length ? 1 : 0),
  ...
  const resourceTrendForKey = (key, windowName) => {
    ...
    const shared = {
      ...
      rate: !staleBeyondWindow && gained >= 0 && elapsed >= RESOURCE_MIN_RATE_WINDOW_MS ? gained / (elapsed / 3600000) : null,
      sampleCount: 2,
      outliers: 0,
      withheld: false,
      windowMs,
    ...
    const localReady = Number.isFinite(local.rate);
    const sharedReady = Number.isFinite(shared.rate);
    if (localReady && (Number(local.latest?.at || 0) >= Number(shared.latest?.at || 0) || !sharedReady || local.sampleCount >= 3)) return local;
    if (sharedReady) return shared;
    return Number(local.latest?.at || 0) >= Number(shared.latest?.at || 0) ? local : shared;
  };
  ...
  const renderXpRankings = () => {
    ...
    return `
      ...
      <div class="so-xp-leaderboards">
        <p class="so-note">Rankings include you, squad members, and favorited profiles in one combined board per activity. Each player uses their reliable Active Sessions rate when available and automatically falls back to a robust multi-snapshot Wall Clock trend. The rate label on every row shows which source is used. All observations come passively from information the game naturally loads.</p>
      ...
      <p class="so-note">Windows are rolling 6-hour, 24-hour, 7-day, or all-observed periods anchored to the present. Active XP needs at least three accepted gain intervals spanning 10 minutes. Wall Clock uses every valid compatible interval, rejects extreme outliers, separates known buff/loadout changes, and uses exact observed level requirements when available. Players without enough natural coverage remain calibrating. RSS requires at least 60 minutes. Ties share the same place.</p>`;
  };
  ...
  const renderXpTrends = () => {
    ...
    if (!history.length && !sharedSummary?.latest) return `${controls}<div class="so-empty">No passive XP observations have been recorded for ${esc(playerName)} yet.</div>`;
    ...
    return `
      ...
      <p class="so-note">${esc(playerName)} • active-session estimates use continuous positive-XP intervals; robust wall-clock trends include idle and offline time. History grows passively from profiles, rankings, squad information, and account data naturally loaded during gameplay.</p>
      ...
      <p class="so-note">Rates appear after at least 10 minutes to avoid misleading extrapolation. Active-session estimates exclude unchanged periods, gaps over 10 minutes, and level crossings whose requirements are unknown. Wall-clock trends use all compatible observations in the selected rolling window. Known level requirements are summed exactly; only missing intermediate requirements are estimated and marked accordingly.</p>`;
  };
  ...
  const renderPvmLab = () => {
    ...
    return `<section class="so-pvm-lab"><h3>Official NPC combat lab</h3>
      ...
      <p class="so-note">The same deterministic random sequence is used for Current and Test, which reduces noise in small before/after changes. Exact individual battles can still vary because combat uses RNG. During the private beta, compare maximum-level predictions with real NPC results after game updates and report material differences with the privacy-safe diagnostic report.</p>
    </section>`;
  };
  ...
  // Base discovery model from the installed Steam wiki; never a net-payout model.
  const DUST_STAR_VALUES = Object.freeze({
    "ringed dwarf": 300, "binary stars": 280, "black hole": 200, "neutron star": 150,
    "o type": 80, "b type": 50, "a type": 40, "f type": 30, "g type": 20, "k type": 10, "m type": 10,
  });
  const DUST_CATALOG_LIMIT = 6000;
  const DUST_SPANS = [6999, 1000, 100];
  const dustNumeric = (value) => ["number", "string"].includes(typeof value) && String(value).trim() !== "" ? finite(value) : null;
  const dustCoordinate = (value) => {
    const n = dustNumeric(value);
    return n !== null && Number.isInteger(n) && n >= 1 && n <= GALAXY_MAX_COORDINATE ? n : null;
  };
  const dustNearestStarter = (x, y, z = 1) => {
    if (dustNumeric(x) === null || dustNumeric(y) === null || Number(z) !== 1) return null;
    let nearest = null, squared = Infinity;
    for (const starter of STARTER_STATIONS) {
      const d = (x - starter.x) ** 2 + (y - starter.y) ** 2;
      if (d < squared) { nearest = starter; squared = d; }
    }
    return { starter: nearest, distance: Math.sqrt(squared) * 10 };
  };
  const dustBaseValue = (x, y, star, bodies, z = 1) => {
    const nearest = dustNearestStarter(x, y, z);
    const starValue = DUST_STAR_VALUES[String(star || "").trim().toLowerCase()];
    const count = dustNumeric(bodies);
    const bodyValue = count !== null && Number.isInteger(count) && count >= 0 && count <= 500 ? count * 20 : null;
    return { nearest, starValue: starValue ?? null, bodyValue,
      base: nearest && starValue !== undefined && bodyValue !== null ? nearest.distance + starValue + bodyValue : null };
  };
  const dustViewport = (x, y, span) => {
    span = DUST_SPANS.includes(span) ? span : DUST_SPANS[0];
    const left = Math.min(GALAXY_MAX_COORDINATE - span, Math.max(1, x - span / 2));
    const bottom = Math.min(GALAXY_MAX_COORDINATE - span, Math.max(1, y - span / 2));
    return { left, bottom, right: left + span, top: bottom + span, span };
  };
  const dustPointFromFraction = (bounds, x, y) => ({
    x: Math.round(bounds.left + Math.max(0, Math.min(1, x)) * bounds.span),
    y: Math.round(bounds.top - Math.max(0, Math.min(1, y)) * bounds.span),
  });
  const dustColor = (value) => {
    const stops = [[17, 29, 49], [56, 39, 91], [143, 54, 105], [226, 120, 70], [255, 223, 124]];
    // Fixed scale across every zoom. 19k covers this bounded Steam map, not a reward cap.
    const t = Math.max(0, Math.min(1, value / 19000)) * (stops.length - 1);
    const i = Math.min(stops.length - 2, Math.floor(t)), f = t - i;
    return stops[i].map((channel, c) => Math.round(channel + (stops[i + 1][c] - channel) * f));
  };
  const dustCatalog = new Map();
  let dustCatalogRevision = 0;
  let dustPublicRows = null, dustPublicKeys = new Set();
  const dustView = { initialized: false, x: 3500, y: 3500, centerX: 3500, centerY: 3500, span: 6999 };
  let dustViewSignature = "", dustRaster = null, dustRasterKey = "", dustJobKey = "", dustJob = 0, dustJobTimer = 0, dustDisposed = false;
  const restoreDustCatalog = (entries) => {
    for (const entry of (Array.isArray(entries) ? entries : []).slice(-DUST_CATALOG_LIMIT)) {
      if (!Array.isArray(entry) || entry.length !== 2 || !/^1:\d+,\d+$/.test(entry[0]) || !Array.isArray(entry[1])) continue;
      const [x, y] = entry[0].slice(2).split(",").map(Number), [star, count, name] = entry[1];
      if (dustCoordinate(x) === null || dustCoordinate(y) === null || !Number.isInteger(count) || count < 0 || count > 500) continue;
      dustCatalog.set(entry[0], [String(star || "").slice(0, 32), count, String(name || "System").slice(0, 80)]);
    }
    dustCatalogRevision++;
  };
  const captureDustSystem = (system, fallbackZ = 1) => {
    if (!system || Number(system.coordinate_z ?? fallbackZ) !== 1 || !Array.isArray(system.bodies) || system.bodies.length > 500) return;
    const x = dustCoordinate(system.coordinate_x), y = dustCoordinate(system.coordinate_y);
    if (x === null || y === null) return;
    const key = `1:${x},${y}`, previous = dustCatalog.get(key);
    const record = [String(system.star || previous?.[0] || "").slice(0, 32), system.bodies.length, String(system.name || system.systemName || previous?.[2] || "System").slice(0, 80)];
    if (previous && previous.every((value, index) => value === record[index])) return;
    dustCatalog.delete(key);
    dustCatalog.set(key, record);
    if (dustCatalog.size > DUST_CATALOG_LIMIT) dustCatalog.delete(dustCatalog.keys().next().value);
    dustCatalogRevision++;
    markDirty();
  };
  const dustDetails = (x, y) => {
    const key = `1:${x},${y}`;
    if (dustPublicRows !== publicAtlas.systems) {
      dustPublicRows = publicAtlas.systems;
      dustPublicKeys = new Set(publicAtlas.systems.map(row => `1:${row.x},${row.y}`));
    }
    const starter = STARTER_STATIONS.find(row => row.x === x && row.y === y);
    const record = dustCatalog.get(key);
    const known = Boolean(starter || record || state.seen[key] || state.systems[key] || dustPublicKeys.has(key));
    const star = record?.[0] || starter?.star || "";
    return { x, y, known, starter: Boolean(starter), name: starter?.name || record?.[2] || "Coordinate point", star,
      bodyCount: record?.[1] ?? null, ...dustBaseValue(x, y, star, record?.[1] ?? null) };
  };
  const dustNumber = (value) => value === null ? "Unknown" : Number(value).toLocaleString(undefined, { maximumFractionDigits: 2 });
  const dustSelectionHtml = () => {
    const row = dustDetails(dustView.x, dustView.y);
    const current = currentCoordinates();
    const travel = current.x !== null && current.y !== null && Number(current.z) === 1 ? Math.hypot(row.x - current.x, row.y - current.y) * 10 : null;
    return `<div class="so-row"><strong>${esc(row.name)} [${row.x}, ${row.y}, 1]</strong>
      <p class="so-dust-status">${row.starter ? "Starter system • no discovery payout" : row.known ? "Known system • not a repeat discovery reward" : "Discovery status unverified • may already be discovered or hidden"}</p>
      <dl class="so-dust-breakdown"><dt>Nearest starter</dt><dd>${esc(row.nearest.starter.name)}</dd>
      <dt>Distance contribution</dt><dd>${dustNumber(row.nearest.distance)} dust</dd>
      <dt>Star contribution</dt><dd>${row.starValue === null ? "Unknown" : `${dustNumber(row.starValue)} (${esc(row.star)})`}</dd>
      <dt>Bodies contribution</dt><dd>${row.bodyValue === null ? "Unknown" : `${dustNumber(row.bodyValue)} (${row.bodyCount} × 20)`}</dd>
      <dt>Full base value</dt><dd>${row.base === null ? "Needs star / body data" : `${dustNumber(row.base)} dust`}</dd></dl>
      <p class="so-note">${dustNumber(row.nearest.distance)} LY from the nearest starter.${travel !== null ? ` ${dustNumber(travel)} LY from you.` : ""} No player bonuses or tax included.</p>
      ${row.base === null ? `<p class="so-note">The distance term is known; missing star/body data is not treated as zero.</p>` : ""}
      <div class="so-inline-actions">${routeDestinationButton({ x: row.x, y: row.y, z: 1, name: row.name === "Coordinate point" ? `Dust target [${row.x}, ${row.y}]` : row.name, kind: "dust" })}</div>
      ${!row.known ? `<p class="so-note">Planning a route does not verify that a new discovery is available here.</p>` : ""}</div>`;
  };
  const renderDustAtlas = () => {
    if (!dustView.initialized) {
      const current = currentCoordinates();
      const x = dustCoordinate(saved.dustView?.x) ?? (Number(current.z) === 1 ? dustCoordinate(current.x) : null) ?? 3500;
      const y = dustCoordinate(saved.dustView?.y) ?? (Number(current.z) === 1 ? dustCoordinate(current.y) : null) ?? 3500;
      Object.assign(dustView, { initialized: true, x, y, centerX: x, centerY: y, span: DUST_SPANS.includes(saved.dustView?.span) ? saved.dustView.span : 6999 });
    }
    return `<div class="so-dust-atlas">
      <p class="so-note">Brighter areas have a larger <strong>distance contribution</strong> to base discovery dust. Steam main galaxy (Z 1).</p>
      <div class="so-dust-toolbar" role="group" aria-label="Heatmap scale">
        <button data-action="dust-zoom" data-span="6999">Galaxy</button><button data-action="dust-zoom" data-span="1000">Region</button><button data-action="dust-zoom" data-span="100">Local</button><button data-action="dust-current">My location</button>
      </div>
      <div class="so-dust-axis" data-dust-y></div>
      <div class="so-dust-map"><canvas data-dust-canvas width="512" height="512" tabindex="0" role="img" aria-label="Distance-based cosmic dust heatmap. Click to select a coordinate; arrow keys move the selection. Exact coordinates can also be entered below.">Heatmap of distance to the nearest starter. Use the coordinate inputs for a textual breakdown.</canvas><div class="so-dust-hover" data-dust-hover hidden></div></div>
      <div class="so-dust-axis" data-dust-x></div>
      <div class="so-dust-gradient" aria-hidden="true"></div><div class="so-dust-axis"><span>0 dust</span><span>Distance term</span><span>19,000 dust</span></div>
      <p class="so-note">◇ Starter · cyan cross: you · white ring: selection. Colors keep the same scale at every zoom; 19,000 is a display scale, not a reward cap.</p>
      <div class="so-dust-coordinates"><label>X<input type="number" min="1" max="7000" step="1" data-dust-input="x" value="${dustView.x}"></label><label>Y<input type="number" min="1" max="7000" step="1" data-dust-input="y" value="${dustView.y}"></label><button data-action="dust-go">Inspect</button></div>
      <p class="so-note" data-dust-feedback role="status"></p>
      <div data-dust-selection aria-live="polite">${dustSelectionHtml()}</div>
      <details class="so-dust-help"><summary>How this map is calculated</summary><p>Base dust = star value + bodies × 20 + LY to the nearest starter. One coordinate step is 10 LY. The distance term is linear and uncapped.</p><p>The smooth color layer shows distance only, not total payout or discovery availability. Exact base totals require the star and full body count, captured from normal game views and the existing journal feed. This release does not download a second full galaxy catalogue.</p><p>Known systems are not repeatable discovery rewards. Unverified coordinates may be hidden or already discovered. Bonus stacking, taxes, and discovery XP are not modeled.</p><p>Rendering is local and cached. No travel, scanning, extra game requests, or cloud uploads are triggered by this tab.</p></details>
    </div>`;
  };
  const drawDustCanvas = (canvas, bounds) => {
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    ctx.clearRect(0, 0, 512, 512);
    if (dustRaster && dustRasterKey === JSON.stringify(bounds)) ctx.drawImage(dustRaster, 0, 0, 512, 512);
    else { ctx.fillStyle = "#111d31"; ctx.fillRect(0, 0, 512, 512); ctx.fillStyle = "#b8cbd5"; ctx.font = "18px Segoe UI"; ctx.fillText("Building cached heatmap…", 30, 250); }
    ctx.strokeStyle = "#d5f2ff20"; ctx.lineWidth = 1;
    for (let i = 1; i < 4; i++) { const p = i * 128; ctx.beginPath(); ctx.moveTo(p, 0); ctx.lineTo(p, 512); ctx.moveTo(0, p); ctx.lineTo(512, p); ctx.stroke(); }
    const point = (x, y) => ({ x: (x - bounds.left) / bounds.span * 512, y: (bounds.top - y) / bounds.span * 512 });
    const visible = (p) => p.x >= 0 && p.x <= 512 && p.y >= 0 && p.y <= 512;
    for (const starter of STARTER_STATIONS) {
      const p = point(starter.x, starter.y); if (!visible(p)) continue;
      ctx.fillStyle = "#0b1829"; ctx.strokeStyle = "#fff0bf"; ctx.lineWidth = 2;
    ...
    const selected = point(dustView.x, dustView.y);
    if (visible(selected)) { ctx.lineWidth = 5; ctx.strokeStyle = "#07111b"; ctx.beginPath(); ctx.arc(selected.x, selected.y, 10, 0, Math.PI * 2); ctx.stroke(); ctx.lineWidth = 2; ctx.strokeStyle = "#fff"; ctx.stroke(); }
    const current = currentCoordinates();
    if (current.x !== null && current.y !== null && Number(current.z) === 1) {
      const p = point(current.x, current.y); if (visible(p)) {
        ctx.lineWidth = 6; ctx.strokeStyle = "#07111b"; ctx.beginPath(); ctx.moveTo(p.x - 8, p.y); ctx.lineTo(p.x + 8, p.y); ctx.moveTo(p.x, p.y - 8); ctx.lineTo(p.x, p.y + 8); ctx.stroke(); ctx.lineWidth = 3; ctx.strokeStyle = "#57ffe2"; ctx.stroke();
      }
    }
  };
  const cancelDustRender = () => { dustJob++; clearTimeout(dustJobTimer); dustJobTimer = 0; dustJobKey = ""; };
  const renderDustRaster = (canvas, bounds) => {
    const key = JSON.stringify(bounds);
    if (dustRasterKey === key && dustRaster) { drawDustCanvas(canvas, bounds); return; }
    if (dustJobKey === key) return;
    cancelDustRender();
    dustJobKey = key;
    const generation = dustJob, raster = document.createElement("canvas");
    raster.width = raster.height = 192;
    const ctx = raster.getContext("2d");
    if (!ctx) { dustJobKey = ""; return; }
    const pixels = ctx.createImageData(192, 192);
    let y = 0;
    const chunk = () => {
      if (dustDisposed || generation !== dustJob) return;
      if (!canvas.isConnected || state.tab !== "dust" || !state.open) { cancelDustRender(); return; }
      const end = Math.min(192, y + 12);
      for (; y < end; y++) for (let x = 0; x < 192; x++) {
        const gx = bounds.left + x / 191 * bounds.span, gy = bounds.top - y / 191 * bounds.span;
        const color = dustColor(dustNearestStarter(gx, gy).distance), index = (y * 192 + x) * 4;
        pixels.data[index] = color[0]; pixels.data[index + 1] = color[1]; pixels.data[index + 2] = color[2]; pixels.data[index + 3] = 255;
      }
      if (y < 192) dustJobTimer = setTimeout(chunk, 0);
      else { ctx.putImageData(pixels, 0, 0); dustRaster = raster; dustRasterKey = key; dustJobKey = ""; dustJobTimer = 0; drawDustCanvas(canvas, bounds); }
    };
    dustJobTimer = setTimeout(chunk, 0);
  };
  const selectDustPoint = (x, y, center = false) => {
    if (dustCoordinate(x) === null || dustCoordinate(y) === null) return false;
    dustView.x = x; dustView.y = y;
    if (center) { dustView.centerX = x; dustView.centerY = y; }
    const root = document.getElementById(ROOT_ID);
    for (const axis of ["x", "y"]) { const input = root?.querySelector(`[data-dust-input="${axis}"]`); if (input) input.value = String(dustView[axis]); }
    markDirty(); dustViewSignature = ""; updateDustAtlas(); return true;
  };
  const handleDustAction = (button) => {
    const root = document.getElementById(ROOT_ID), feedback = root?.querySelector("[data-dust-feedback]");
    if (feedback) feedback.textContent = "";
    if (button.dataset.action === "dust-zoom") {
      const span = Number(button.dataset.span); if (!DUST_SPANS.includes(span)) return;
      dustView.span = span; dustView.centerX = dustView.x; dustView.centerY = dustView.y;
      markDirty(); dustViewSignature = ""; updateDustAtlas();
    } else if (button.dataset.action === "dust-current") {
      const current = currentCoordinates();
      if (Number(current.z) !== 1 || !selectDustPoint(current.x, current.y, true)) {
        if (feedback) feedback.textContent = "Your main-galaxy position is unavailable. This atlas covers Z 1 only.";
      }
    } else if (button.dataset.action === "dust-go") {
      const x = dustCoordinate(root.querySelector('[data-dust-input="x"]').value), y = dustCoordinate(root.querySelector('[data-dust-input="y"]').value);
      if (!selectDustPoint(x, y, true) && feedback) feedback.textContent = "Enter whole-number X and Y coordinates from 1 to 7000.";
    }
  };
  const updateDustAtlas = () => {
    if (state.tab !== "dust" || !state.open || dustDisposed) { if (dustJobKey) cancelDustRender(); return; }
    const root = document.getElementById(ROOT_ID), atlas = root?.querySelector(".so-dust-atlas"), canvas = atlas?.querySelector("[data-dust-canvas]");
    if (!canvas) return;
    if (!canvas.dataset.bound) {
      canvas.dataset.bound = "1";
      const cursorPoint = (event) => { const rect = canvas.getBoundingClientRect(); return dustPointFromFraction(dustViewport(dustView.centerX, dustView.centerY, dustView.span), (event.clientX - rect.left) / rect.width, (event.clientY - rect.top) / rect.height); };
      canvas.addEventListener("click", event => { const p = cursorPoint(event); selectDustPoint(p.x, p.y); });
      canvas.addEventListener("pointermove", event => { const p = cursorPoint(event), nearest = dustNearestStarter(p.x, p.y), hover = atlas.querySelector("[data-dust-hover]"); hover.hidden = false; hover.textContent = `[${p.x}, ${p.y}] · ${dustNumber(nearest.distance)} distance dust · ${nearest.starter.name}`; });
      canvas.addEventListener("pointerleave", () => { atlas.querySelector("[data-dust-hover]").hidden = true; });
      canvas.addEventListener("keydown", event => {
        const delta = { ArrowLeft: [-1, 0], ArrowRight: [1, 0], ArrowUp: [0, 1], ArrowDown: [0, -1] }[event.key];
        if (!delta) return; event.preventDefault(); event.stopPropagation();
        const step = event.shiftKey ? 10 : 1;
        selectDustPoint(Math.max(1, Math.min(7000, dustView.x + delta[0] * step)), Math.max(1, Math.min(7000, dustView.y + delta[1] * step)));
      });
      atlas.querySelectorAll("[data-dust-input]").forEach(input => input.addEventListener("keydown", event => { if (event.key === "Enter") { event.preventDefault(); handleDustAction(atlas.querySelector('[data-action="dust-go"]')); } }));
      dustViewSignature = "";
    }
    const signature = JSON.stringify([dustView, currentCoordinates(), dustCatalogRevision, publicAtlas.fetchedAt, dustDetails(dustView.x, dustView.y).known, state.routePlanner.destinations.map(row => row.key)]);
    const bounds = dustViewport(dustView.centerX, dustView.centerY, dustView.span);
    const boundsKey = JSON.stringify(bounds);
    if (signature === dustViewSignature && ((dustRaster && dustRasterKey === boundsKey) || dustJobKey === boundsKey)) return;
    dustViewSignature = signature;
    atlas.querySelector("[data-dust-y]").textContent = `Y ↑ ${dustNumber(bounds.bottom)} – ${dustNumber(bounds.top)}`;
    atlas.querySelector("[data-dust-x]").innerHTML = `<span>${dustNumber(bounds.left)}</span><span>X →</span><span>${dustNumber(bounds.right)}</span>`;
    atlas.querySelectorAll('[data-action="dust-zoom"]').forEach(button => button.setAttribute("aria-pressed", String(Number(button.dataset.span) === dustView.span)));
    atlas.querySelector("[data-dust-selection]").innerHTML = dustSelectionHtml();
    drawDustCanvas(canvas, bounds); renderDustRaster(canvas, bounds);
  };
  ...
  const renderEngineAlerts = () => {
    ...
    const desktopPermission = nativeAlertsAvailable()
      ? "Windows tray — works while the game is in the background"
      : "Notification" in window ? `Game permission: ${window.Notification.permission}` : "unsupported";
    return `
      <div class="so-alerts">
        ...
        <section>
          ...
          <div class="so-alert-options">
            ${toggle("sound", "Audio chime", nativeAlertsAvailable() ? "Three tones, including while another app is active" : "Three short ascending tones")}
            ${toggle("toast", "In-game notification", "A centered message over the game")}
            ${toggle("flash", "Screen / taskbar flash", nativeAlertsAvailable() ? "Gold in-game pulse or taskbar attention in the background" : "A brief gold edge pulse")}
            ${toggle("badge", "Persistent ready badge", "Stays visible until dismissed or the next cooldown")}
            ${toggle("desktop", "Desktop notification", desktopPermission)}
        ...
        <p class="so-note">${nativeAlertsAvailable() ? "Keep WookiesStarwatch.exe running in the notification area. Once the map countdown arms the monitor, the Windows launcher keeps time and can alert while the game is minimized or behind another app." : "This launcher uses in-game alerts only. Install launcher 2.1.0 or newer for reliable Windows background alerts."} Monitoring is local and passive: it never presses travel or sends a request to the game server.</p>
      </div>`;
  };
  ...
  const renderSync = () => {
    ...
    const consentAccepted = Boolean(state.sync.privacyAcceptedAt);
    ...
    return `
      <div class="so-sync">
        ...
        <label class="so-sync-toggle so-privacy-consent">
          <input type="checkbox" data-action="privacy-consent" ${consentAccepted ? "checked" : ""}>
          <span><strong>I understand what shared sync stores</strong><small>Profile/XP/RSS snapshots, favorites, account map discoveries, and—only if separately enabled—anonymous completed-battle evidence are sent to the independent companion service. API keys, Steam passwords, game cookies, chat, inventory materials, and raw API responses are never uploaded.</small></span>
        </label>
        <div class="so-account-card ${steamConnected ? "connected" : ""}">
          ...
          <button class="so-steam-button" data-action="sync-steam" ${state.sync.authBusy || !owner || !consentAccepted ? "disabled" : ""}>${steamConnected ? "Reconnect Steam" : pendingSteam ? "Restart connection" : "Connect with Steam"}</button>
          ...
          ${steamConnected ? `<button class="so-link-secondary danger" data-action="sync-account-delete">Delete linked cloud account data</button>` : ""}
        </div>
        <label class="so-sync-toggle">
          <input type="checkbox" data-action="sync-enabled" ${state.sync.enabled ? "checked" : ""} ${consentAccepted ? "" : "disabled"}>
        ...
        <label class="so-sync-toggle">
          <input type="checkbox" data-action="battle-sharing" ${state.sync.battleSharing ? "checked" : ""} ${state.sync.enabled && consentAccepted ? "" : "disabled"}>
        ...
        <details class="so-sync-advanced so-privacy-details">
          <summary>Privacy, retention, and deletion</summary>
          <p>Steam linking stores a Steam identifier, the detected game-account key, a hashed device credential, favorites, and account-specific map discoveries. Shared observations keep seven days of raw detail, then use compact hourly/lifetime summaries.</p>
          <p>Disconnecting revokes only this device. <strong>Delete linked cloud account data</strong> removes the Steam/device link, favorites, map index, and raw submissions still attributable to this account. De-identified pooled statistics and copies already merged onto another player’s device cannot be recalled. Local overlay data stays on this PC until you clear it in Rules & Privacy.</p>
        </details>
        ...
        <button class="so-alert-test" data-action="diagnostic-copy">Copy privacy-safe diagnostic report</button>
      </div>`;
  };
  ...
  const renderAbout = () => `
    <div class="so-about">
      ...
      <div class="so-feature-index">
        <div><b>Nodes</b><span>event radar, auto-saved perfect nodes and portal-aware shortest routes</span></div>
      ...
      <h3>Private-beta accuracy limits</h3>
      <p>PvP estimates remain experimental until enough comparable battles have been observed, and hidden opponent bonuses remain unknown. NPC results use the installed client’s simulator but still require real-battle comparison when the game changes. XP/RSS rankings need sufficient elapsed history, and routes are optimal only within the systems, fuel values, portals, and mechanics currently exposed to the overlay.</p>
      <p>When reporting a problem, copy the privacy-safe diagnostic report from Sync. It contains versions, statuses, queue sizes, and feature counts—but no player names, coordinates, profile contents, keys, or credentials.</p>
      <h3>Map legend</h3>
      ...
      <div class="so-version">PRIVATE BETA • Wookie's Starwatch v${VERSION}</div>
    </div>`;
  ...
  const ensureUi = () => {
    ...
    style.textContent = `
      ...
      #${ROOT_ID} .so-tabs{display:flex;flex:0 0 68px;min-width:68px;flex-direction:column;background:#091824;border-right:1px solid #203d50;overflow-y:auto;scrollbar-width:thin}
      ...
      #${ROOT_ID} .so-perfect-result .so-row-main{display:grid;grid-template-columns:minmax(0,1fr);gap:3px}#${ROOT_ID} .so-perfect-result .so-row-main>*{white-space:normal;overflow-wrap:anywhere}
      #${ROOT_ID} .so-dust-atlas{min-width:0}#${ROOT_ID} .so-dust-toolbar{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:8px}#${ROOT_ID} .so-dust-toolbar button,#${ROOT_ID} .so-dust-coordinates button{border:1px solid #34566e;border-radius:5px;background:#132d40;color:#b3d2df;padding:6px 8px;font-size:11px;cursor:pointer}#${ROOT_ID} .so-dust-toolbar button[aria-pressed="true"]{background:#344055;color:#ffe49f;border-color:#a38e5c}
      #${ROOT_ID} .so-dust-map{position:relative;aspect-ratio:1;background:#111d31;border:1px solid #395269;border-radius:7px;overflow:hidden}#${ROOT_ID} .so-dust-map canvas{display:block;width:100%;height:100%;cursor:crosshair;outline-offset:-3px}#${ROOT_ID} .so-dust-hover{position:absolute;bottom:6px;left:6px;right:6px;padding:6px 8px;background:#081421ed;border:1px solid #50677b;border-radius:4px;color:#ffe6ae;font-size:11px;pointer-events:none}#${ROOT_ID} .so-dust-hover[hidden]{display:none}
      #${ROOT_ID} .so-dust-axis{display:flex;justify-content:space-between;gap:6px;color:#a3bbca;font-size:11px;margin:5px 0}#${ROOT_ID} .so-dust-gradient{height:9px;margin-top:10px;border-radius:5px;background:linear-gradient(90deg,#111d31,#38275b,#8f3669,#e27846,#ffdf7c)}
      #${ROOT_ID} .so-dust-coordinates{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) auto;align-items:end;gap:7px}#${ROOT_ID} .so-dust-coordinates label{display:grid;gap:3px;min-width:0;color:#a3bbca;font-size:11px}#${ROOT_ID} .so-dust-coordinates input{width:100%;min-width:0;padding:6px;color:#e2f5ff;background:#0d2130;border:1px solid #395269;border-radius:4px;font:inherit;font-size:12px}
      #${ROOT_ID} .so-dust-breakdown{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.2fr);gap:7px 9px;font-size:11px;margin:10px 0}#${ROOT_ID} .so-dust-breakdown dt{color:#94b0bf}#${ROOT_ID} .so-dust-breakdown dd{margin:0;text-align:right;color:#fff0c9;overflow-wrap:anywhere;font-variant-numeric:tabular-nums}#${ROOT_ID} .so-dust-status{font-size:11px;color:#ecc882;line-height:1.5;margin:6px 0}#${ROOT_ID} .so-dust-help{margin-top:14px;color:#93afc0;font-size:11px}#${ROOT_ID} .so-dust-help summary{cursor:pointer;color:#bfdae4;padding:6px 0}#${ROOT_ID} [data-dust-selection] strong{overflow-wrap:anywhere}
      ...
      #${ROOT_ID} .so-tool-controls{display:grid;grid-template-columns:minmax(0,1fr);gap:7px}.so-tool-controls label,.so-market-tools label,.so-build-selectors label,.so-build-mods label{display:flex;min-width:0;flex-direction:column;gap:3px;color:#7897a8;font-size:9px}.so-tool-controls input,.so-tool-controls select,.so-market-tools input,.so-market-tools select,.so-build-selectors select,.so-build-mods input{min-width:0;width:100%;border:1px solid #31536a;border-radius:5px;background:#0b1e2b;color:#dff;padding:5px}.so-section-heading{display:flex;align-items:center;justify-content:space-between;gap:8px}.so-section-heading h3{margin:0}.so-section-heading button{border:1px solid #69474b;border-radius:5px;background:#2b1c24;color:#efadb4;padding:4px 7px;font-size:9px;cursor:pointer}.so-route-toggle{display:grid;grid-template-columns:18px minmax(0,1fr);align-items:start;gap:7px;margin:7px 0 9px;padding:7px 8px;border:1px solid #31536a;border-radius:6px;background:#0d2130}.so-route-toggle input{width:14px;height:14px;margin:2px 0 0;accent-color:#4ddfce}.so-route-toggle span{display:flex;min-width:0;flex-direction:column}.so-route-toggle strong{color:#d9f5f2;font-size:10px}.so-route-toggle small{color:#7897a8;font-size:8px}.so-route-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px;margin-bottom:7px}.so-route-summary>div{display:flex;min-width:0;flex-direction:column;padding:7px;border:1px solid #285066;border-radius:6px;background:#0b1e2b}.so-route-summary span{color:#7897a8;font-size:8px;text-transform:uppercase}.so-route-summary strong{overflow:hidden;text-overflow:ellipsis;color:#73e8db;font-size:13px}.so-route-summary small{color:#7897a8;font-size:8px}.so-route-warning{margin:6px 0;padding:7px 8px;border:1px solid #84682d;border-radius:6px;background:#302812;color:#edce79;font-size:9px}.so-route-footnote{margin:7px 1px 0;color:#718d99;font-size:8px}.so-route-list,.so-event-list,.so-timeline{display:flex;flex-direction:column;gap:6px}.so-route-row{display:grid;grid-template-columns:23px minmax(0,1fr) auto;gap:7px;align-items:center;padding:7px;border:1px solid #285066;border-radius:7px;background:#102636}.so-route-row>b{display:grid;width:21px;height:21px;place-items:center;border-radius:50%;background:#24655f;color:#dffffa}.so-route-row>div{display:flex;min-width:0;flex-direction:column}.so-route-row strong{color:#dff}.so-route-row span{color:#7897a8;font-size:9px;line-height:1.45}.so-route-row .so-route-actions{display:flex;flex-direction:row;align-items:center;gap:4px}.so-route-row button,.so-event button,.so-inline-actions button{border:1px solid #355f72;border-radius:5px;background:#123044;color:#a9d7df;padding:4px 6px;font-size:9px;cursor:pointer}.so-route-row button:disabled,.so-event button:disabled,.so-inline-actions button:disabled{opacity:.58;cursor:default}.so-route-itinerary{grid-column:2/-1;max-height:240px;margin:2px 0 0;padding:7px;overflow:auto;border:1px solid #244c60;border-radius:5px;background:#081924;color:#9fc3cf;font:8px/1.55 Consolas,monospace;white-space:pre}.so-inline-actions{display:flex;justify-content:flex-end;gap:5px;margin-top:6px}.so-event-feed{display:flex;flex-direction:column;gap:2px;margin:7px 0;padding:7px 8px;border:1px solid #31536a;border-left:3px solid #4ddcca;border-radius:6px;background:#0d2130}.so-event-feed strong{color:#dff;font-size:10px}.so-event-feed span,.so-event-feed small{color:#7897a8;font-size:9px}.so-event-feed.stale{border-left-color:#efb64f}.so-event-feed.error{border-left-color:#ed6974}.so-event-feed.error strong{color:#ffadb4}.so-event{display:grid;grid-template-columns:47px minmax(0,1fr) auto;align-items:center;gap:7px;padding:8px;border:1px solid #375468;border-left-width:3px;border-radius:7px;background:#102636}.so-event.rune{border-left-color:#c87aff}.so-event.dungeon{border-left-color:#ff6b73}.so-event.solo{border-left-color:#67e59f}.so-event.boss{border-left-color:#efb64f}.so-event-kind{color:#7f9cab;font-size:8px;font-weight:900}.so-event>div{display:flex;min-width:0;flex-direction:column}.so-event strong{color:#e5faff}.so-event small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#7897a8;font-size:9px}
      ...
      .galaxy-minimap .${MINI_MAP_ROUTE_CLASS}{position:absolute;inset:0;z-index:2;width:100%;height:100%;overflow:visible;pointer-events:none}.galaxy-minimap .so-mini-route-flight{stroke:#55ebdc;stroke-width:1.6;vector-effect:non-scaling-stroke;filter:drop-shadow(0 0 2px #25cbbb)}.galaxy-minimap .so-mini-route-portal{stroke:#dc8aff;stroke-width:1.8;stroke-dasharray:5 4;vector-effect:non-scaling-stroke}.galaxy-minimap .so-mini-route-jump{fill:#5af0df;stroke:#06242b;stroke-width:1;vector-effect:non-scaling-stroke}.galaxy-minimap .so-mini-route-start circle{fill:#9d680b;stroke:#ffe07b;stroke-width:2;vector-effect:non-scaling-stroke}.galaxy-minimap .so-mini-route-destination circle{fill:#9b6408;stroke:#ffe07b;stroke-width:2;vector-effect:non-scaling-stroke}.galaxy-minimap .so-mini-route-start text,.galaxy-minimap .so-mini-route-destination text{fill:#fff1a8;stroke:#061018;stroke-width:14;font:800 190px Arial,sans-serif;text-anchor:middle;paint-order:stroke;pointer-events:none}
    ...
    root.innerHTML = `
      ...
      <button class="so-toggle" data-action="toggle">Starwatch</button>
      <div class="so-panel">
        <nav class="so-tabs" aria-label="Overlay sections">
          <div class="so-brand"><strong>WS</strong><span>Watch</span></div>
          <button data-action="tab" data-tab="nodes">Nodes</button>
          <button data-action="tab" data-tab="dust">Dust</button>
        ...
        <div class="so-workspace">
          <header class="so-title">
            <div class="so-title-copy"><strong>Wookie's Starwatch</strong><span>Unofficial Stellar Odyssey companion</span></div>
    ...
    root.addEventListener("click", (event) => {
      ...
      if (action.startsWith("dust-")) {
        handleDustAction(button);
        ...
        const destination = {
          key: String(button.dataset.routeKey || ""),
          name: String(button.dataset.routeName || "Destination").slice(0, 100),
          kind: String(button.dataset.routeKind || "destination").slice(0, 30),
          x: finite(button.dataset.routeX),
          y: finite(button.dataset.routeY),
          z: finite(button.dataset.routeZ ?? 1) ?? 1,
          addedAt: Date.now(),
        };
        if (destination.x === null || destination.y === null) return;
        destination.key ||= routeCoordinateKey(destination);
        if (!state.routePlanner.destinations.some((row) => row.key === destination.key)) {
          if (state.routePlanner.destinations.length >= ROUTE_MAX_DESTINATIONS) {
            showEngineToast(`Route is limited to ${ROUTE_MAX_DESTINATIONS} destinations so the exact optimizer stays fast.`, true);
            return;
          }
          state.routePlanner.destinations.push(destination);
        }
        state.tab = "nodes";
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
      } else if (action === "remove-route-stop") {
        const key = String(button.dataset.routeKey || "");
        state.routePlanner.destinations = state.routePlanner.destinations.filter((row) => row.key !== key);
        if (state.routePlanner.expandedLeg === key) state.routePlanner.expandedLeg = "";
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
      } else if (action === "toggle-route-leg") {
        const key = String(button.dataset.routeKey || "");
        state.routePlanner.expandedLeg = state.routePlanner.expandedLeg === key ? "" : key;
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
      } else if (action === "clear-route") {
        state.routePlanner.destinations = [];
        state.routePlanner.expandedLeg = "";
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
        ...
        revokeSteamLink();
      } else if (action === "sync-account-delete") {
        const confirmation = window.prompt("This permanently removes the linked account, favorites, map index, and attributable raw submissions from the companion service. Local overlay data stays on this PC. Type DELETE to continue.");
        if (confirmation === "DELETE") deleteCloudAccount();
      } else if (action === "diagnostic-copy") {
        copyDiagnosticReport();
      } else if (action === "sync-generate") {
        if (!state.sync.privacyAcceptedAt) {
          state.sync.status = "setup";
          state.sync.message = "Accept the shared-data notice before creating a recovery connection";
          state.lastPanelSignature = "";
          renderPanel();
          return;
        }
        ...
        if (!state.sync.privacyAcceptedAt) {
          state.sync.status = "setup";
          state.sync.message = "Accept the shared-data notice before enabling recovery sync";
          state.lastPanelSignature = "";
          renderPanel();
          return;
        }
        ...
        dustCatalog.clear(); dustCatalogRevision++; dustView.initialized = false; dustViewSignature = ""; cancelDustRender();
        state.systems = {};
        ...
        state.routePlanner = { destinations: [], usePortals: true, showOnMaps: true, expandedLeg: "" };
        state.operations = {};
    ...
    root.addEventListener("change", (event) => {
      if (event.target.matches('[data-action="select-profile"]')) {
        ...
        state.lastPanelSignature = "";
        renderPanel();
      } else if (event.target.matches('[data-action="node-resource"]')) {
        const resource = String(event.target.value || "all");
        state.nodeFinder.resource = ["rocky", "icy", "gas", "crystal"].includes(resource) ? resource : "all";
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
        ...
        state.lastPanelSignature = "";
        renderPanel();
      } else if (event.target.matches('[data-action="route-use-portals"]')) {
        state.routePlanner.usePortals = event.target.checked === true;
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
      } else if (event.target.matches('[data-action="route-show-on-maps"]')) {
        state.routePlanner.showOnMaps = event.target.checked === true;
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
        renderMapMarkers();
        ...
        if (!state.sync.privacyAcceptedAt) {
          ...
          state.sync.enabled = false;
          state.sync.status = "setup";
          state.sync.message = "Accept the shared-data notice before enabling shared sync";
          state.lastPanelSignature = "";
          renderPanel();
          return;
        }
      ...
      } else if (event.target.matches('[data-action="privacy-consent"]')) {
        state.sync.privacyAcceptedAt = event.target.checked ? Date.now() : 0;
        if (!state.sync.privacyAcceptedAt) {
          state.sync.enabled = false;
          state.sync.battleSharing = false;
          state.sync.status = "idle";
          state.sync.message = "Shared sync is off";
        } else {
          state.sync.status = state.sync.enabled ? "setup" : "idle";
          state.sync.message = state.sync.enabled ? "Shared-data notice accepted" : "Shared-data notice accepted • connect Steam when ready";
        }
        markDirty();
        state.lastPanelSignature = "";
        renderPanel();
  ...
  // Keep the live anchor and its ancestors connected: detaching a focused select
  // closes Chromium's native popup even if it is immediately reinserted.
  const replaceAroundAnchor = (parent, anchor, incoming, incomingAnchor) => {
    for (const child of [...parent.childNodes]) {
      if (child !== anchor) child.remove();
    }
    let afterAnchor = false;
    for (const child of [...incoming.childNodes]) {
      if (child === incomingAnchor) { afterAnchor = true; continue; }
      if (afterAnchor) parent.appendChild(child);
      else parent.insertBefore(child, anchor);
    }
  };

  const updatePanelContent = (content, html) => {
    // Keep the heatmap canvas and coordinate drafts stable across unrelated live updates.
    if (state.tab === "dust" && content.querySelector(".so-dust-atlas")) return;
    const finder = state.tab === "nodes" && content.querySelector('[data-section="perfect-finder"]');
    const controls = finder && finder.querySelector("[data-node-filters]");
    if (!controls) { content.innerHTML = html; return; }
    const template = document.createElement("template");
    template.innerHTML = html;
    const nextFinder = template.content.querySelector('[data-section="perfect-finder"]');
    const nextControls = nextFinder && nextFinder.querySelector("[data-node-filters]");
    if (!nextControls) { content.innerHTML = html; return; }

    // Preserve the dropdown popup, keyboard focus, and an unfinished search draft.
    // All surrounding results, distances, and countdowns continue refreshing.
    for (const action of ["node-resource", "node-query"]) {
      const selector = `[data-action="${action}"]`;
      const control = controls.querySelector(selector);
      const nextControl = nextControls.querySelector(selector);
      if (control && nextControl && document.activeElement !== control && control.value !== nextControl.value) {
        control.value = nextControl.value;
      }
    }
    replaceAroundAnchor(finder, controls, nextFinder, nextControls);
    replaceAroundAnchor(content, finder, template.content, nextFinder);
  };

  const renderPanel = () => {
    const root = ensureUi();
    syncDrawerLayout();
    if (state.tab !== "dust" || !state.open) { if (dustJobKey) cancelDustRender(); }
    const signature = JSON.stringify({
      ...
      galaxyTools: [state.nodeFinder, state.routePlanner, state.officialApi.dungeons, state.officialApi.soloDungeons, state.officialApi.activeRunes, state.officialApi.passiveEvents],
      perfectIndex: state.tab === "nodes" ? [nodeDataRevision, currentCoordinates(), publicAtlas.checkedAt, publicAtlas.refreshing, publicAtlas.stale, publicAtlas.message] : null,
      dust: state.tab === "dust" ? [dustView, currentCoordinates(), dustCatalogRevision, publicAtlas.fetchedAt] : null,
    ...
    updatePanelContent(root.querySelector(".so-content"), panelContent());
    updateDustAtlas();
    renderEngineBadge();
  };
  ...
  let routeMapCache = { signature: "", geometry: null };
  let nativeRouteShape = null;
  let nativeRouteLastGroup = null;
  let nativeRouteLastScale = null;
  let nativeRouteLastSignature = "";

  const routeMapGeometry = () => {
    const context = routeGameContext();
    const signature = JSON.stringify([
      state.routePlanner.showOnMaps,
      state.routePlanner.usePortals,
      context.current,
      context.totalFuel,
      context.fuelEfficiency,
      state.routePlanner.destinations.map((row) => [row.key, row.x, row.y, row.z]),
      context.stations.filter((station) => station.portalKnown && station.portalEnabled).map((station) => [station.key, station.portalLevel]),
    ]);
    if (routeMapCache.signature === signature) return routeMapCache.geometry;
    if (!state.routePlanner.showOnMaps || !state.routePlanner.destinations.length || context.current.x === null || context.current.y === null) {
      routeMapCache = { signature, geometry: null };
      return null;
    }
    const plan = plannedRoute();
    ...
    plan.legs.forEach((leg, legIndex) => {
      for (const step of leg.steps) {
        if (step.type === "teleport") portals.push({ from: plain(step.from), to: plain(step.to), leg: legIndex + 1 });
        else jumps.push({ from: plain(step.from), to: plain(step.to), number: ++jumpNumber, leg: legIndex + 1 });
      }
      destinations.push({ ...plain(leg.destination), leg: legIndex + 1 });
    });
    routeMapCache = {
      signature,
      geometry: {
        signature,
        z: Number(plan.origin.z ?? 1),
        origin: plain(plan.origin),
        jumps,
        portals,
        destinations,
      },
    };
    return routeMapCache.geometry;
  };

  const clearNativeRoutePath = () => {
    const layer = nativeRouteShape?.getLayer?.();
    nativeRouteShape?.destroy?.();
    nativeRouteShape = null;
    nativeRouteLastGroup = null;
    nativeRouteLastScale = null;
    nativeRouteLastSignature = "";
    layer?.batchDraw?.();
  };

  const drawNativeRouteShape = (context, shape) => {
    const canvas = context?._context;
    const geometry = shape.getAttr("soIntelGeometry");
    const scale = Math.max(0.001, finite(shape.getAttr("soIntelScale")) || 1);
    const stepX = finite(shape.getAttr("soIntelStepX")) || 50;
    const stepY = finite(shape.getAttr("soIntelStepY")) || 30;
    if (!canvas || !geometry) return;
    const point = (row) => ({ x: Number(row.x) * stepX, y: -Number(row.y) * stepY });
    const lineWidth = 2.2 / scale;
    const badgeRadius = 7 / scale;
    const fontSize = 8 / scale;
    const drawBadge = (row, label, fill, stroke, radius = badgeRadius) => {
      const position = point(row);
      canvas.beginPath();
      canvas.arc(position.x, position.y, radius, 0, Math.PI * 2);
      canvas.fillStyle = fill;
      canvas.fill();
      canvas.lineWidth = 1.2 / scale;
      canvas.strokeStyle = stroke;
      canvas.stroke();
      canvas.fillStyle = "#f4ffff";
      canvas.font = `800 ${fontSize}px Arial, sans-serif`;
      canvas.textAlign = "center";
      canvas.textBaseline = "middle";
      canvas.fillText(String(label), position.x, position.y + 0.35 / scale);
    };
    const drawTag = (row, label, color, offsetY = -14) => {
      const position = point(row);
      canvas.font = `800 ${9 / scale}px Arial, sans-serif`;
      const width = canvas.measureText(label).width + 8 / scale;
      const height = 14 / scale;
      const x = position.x - width / 2;
      const y = position.y + offsetY / scale - height / 2;
      canvas.fillStyle = "rgba(4,15,24,0.9)";
      ...
      canvas.textAlign = "center";
      canvas.textBaseline = "middle";
      canvas.fillText(label, position.x, position.y + offsetY / scale);
    };
    canvas.save();
    canvas.lineCap = "round";
    canvas.lineJoin = "round";
    canvas.shadowColor = "rgba(55,236,219,0.72)";
    canvas.shadowBlur = 5 / scale;
    canvas.strokeStyle = "rgba(76,235,221,0.9)";
    canvas.lineWidth = lineWidth;
    canvas.beginPath();
    for (const jump of geometry.jumps) {
      const from = point(jump.from);
      const to = point(jump.to);
      canvas.moveTo(from.x, from.y);
      canvas.lineTo(to.x, to.y);
    }
    canvas.stroke();
    canvas.shadowColor = "transparent";
    canvas.setLineDash([7 / scale, 5 / scale]);
    canvas.strokeStyle = "rgba(211,126,255,0.95)";
    canvas.lineWidth = 2.4 / scale;
    canvas.beginPath();
    for (const portal of geometry.portals) {
      const from = point(portal.from);
      const to = point(portal.to);
      canvas.moveTo(from.x, from.y);
      canvas.lineTo(to.x, to.y);
    }
    canvas.stroke();
    canvas.setLineDash([]);
    for (const jump of geometry.jumps) drawBadge(jump.to, jump.number, "rgba(12,74,81,0.94)", "#6effec", String(jump.number).length >= 3 ? 9 / scale : badgeRadius);
    drawBadge(geometry.origin, "S", "rgba(117,76,8,0.96)", "#ffd266", 8 / scale);
    drawTag(geometry.origin, "START", "#ffd266");
    for (const portal of geometry.portals) {
      drawBadge(portal.from, "P", "rgba(82,28,116,0.96)", "#df9aff", 8 / scale);
      drawBadge(portal.to, "P", "rgba(82,28,116,0.96)", "#df9aff", 8 / scale);
    }
    for (const destination of geometry.destinations) {
      drawBadge(destination, `D${destination.leg}`, "rgba(126,79,4,0.96)", "#ffd76a", 10 / scale);
      drawTag(destination, `D${destination.leg} ${destination.name}`, "#ffd76a", 16);
    }
    canvas.restore();
  };

  const syncNativeRoutePath = () => {
    const map = nativeGlowContext?.map;
    const groupNode = nativeGlowContext?.groupNode;
    if (!map || !groupNode || groupNode.isDestroyed?.()) return;
    const geometry = routeMapGeometry();
    const currentZ = finite(map.props?.currentZ) ?? 1;
    if (!geometry || Number(geometry.z) !== Number(currentZ)) {
      clearNativeRoutePath();
      return;
    }
    const Shape = window.Konva?.Shape;
    if (!Shape) return;
    const q = getApp()?.config?.globalProperties?.$q;
    const compact = Boolean(q?.screen?.lt?.md);
    const stepX = compact ? 32 : 50;
    const stepY = compact ? 19 : 30;
    const scale = finite(groupNode.scaleX?.()) ?? finite(groupNode.scale?.()?.x) ?? 1;
    const requiresNewShape = !nativeRouteShape || nativeRouteShape.getParent?.() !== groupNode || nativeRouteLastSignature !== geometry.signature || nativeRouteShape.getAttr("soIntelStepX") !== stepX || nativeRouteShape.getAttr("soIntelStepY") !== stepY;
    let changed = requiresNewShape;
    if (requiresNewShape) {
      clearNativeRoutePath();
      nativeRouteShape = new Shape({
        name: "so-intel-planned-route",
        soIntelGeometry: geometry,
        soIntelScale: scale,
        soIntelStepX: stepX,
        soIntelStepY: stepY,
        sceneFunc: drawNativeRouteShape,
        listening: false,
        perfectDrawEnabled: false,
      });
      groupNode.add(nativeRouteShape);
      nativeRouteShape.moveToTop();
    } else if (Math.abs(scale - (nativeRouteLastScale ?? scale)) > 0.001) {
      nativeRouteShape.setAttr("soIntelScale", scale);
      changed = true;
    }
    nativeRouteLastGroup = groupNode;
    nativeRouteLastScale = scale;
    nativeRouteLastSignature = geometry.signature;
    if (changed) nativeRouteShape.getLayer?.()?.batchDraw?.();
  };

  const nativeRouteNeedsSync = () => {
    const groupNode = nativeGlowContext?.groupNode;
    if (!groupNode || groupNode.isDestroyed?.()) return false;
    const scale = finite(groupNode.scaleX?.()) ?? finite(groupNode.scale?.()?.x) ?? 1;
    return nativeRouteShape && (nativeRouteShape.getParent?.() !== groupNode || Math.abs(scale - (nativeRouteLastScale ?? scale)) > 0.001);
  };

  const renderMiniMapRoute = (geometry, currentZ) => {
    document.querySelectorAll(`.${MINI_MAP_ROUTE_CLASS}`).forEach((element) => {
      if (!element.closest(".galaxy-minimap")) element.remove();
    });
    const minimap = document.querySelector(".galaxy-minimap");
    if (!minimap || !geometry || Number(geometry.z) !== Number(currentZ)) {
      minimap?.querySelector(`.${MINI_MAP_ROUTE_CLASS}`)?.remove();
      return;
    }
    let svg = minimap.querySelector(`.${MINI_MAP_ROUTE_CLASS}`);
    if (!svg) {
      svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
      svg.classList.add(MINI_MAP_ROUTE_CLASS);
      svg.setAttribute("viewBox", `0 0 ${GALAXY_MAX_COORDINATE} ${GALAXY_MAX_COORDINATE}`);
      svg.setAttribute("preserveAspectRatio", "none");
      minimap.appendChild(svg);
    }
    if (svg.dataset.routeSignature === geometry.signature) return;
    const x = (row) => Number(row.x);
    const y = (row) => GALAXY_MAX_COORDINATE - Number(row.y);
    const flightLines = geometry.jumps.map((jump) => `<line class="so-mini-route-flight" x1="${x(jump.from)}" y1="${y(jump.from)}" x2="${x(jump.to)}" y2="${y(jump.to)}"/>`).join("");
    const portalLines = geometry.portals.map((portal) => `<line class="so-mini-route-portal" x1="${x(portal.from)}" y1="${y(portal.from)}" x2="${x(portal.to)}" y2="${y(portal.to)}"/>`).join("");
    const jumps = geometry.jumps.map((jump) => `<circle class="so-mini-route-jump" cx="${x(jump.to)}" cy="${y(jump.to)}" r="35"/>`).join("");
    const destinations = geometry.destinations.map((destination) => `<g class="so-mini-route-destination"><circle cx="${x(destination)}" cy="${y(destination)}" r="85"/><text x="${x(destination)}" y="${y(destination) - 120}">D${destination.leg}</text></g>`).join("");
    svg.innerHTML = `${flightLines}${portalLines}${jumps}<g class="so-mini-route-start"><circle cx="${x(geometry.origin)}" cy="${y(geometry.origin)}" r="85"/><text x="${x(geometry.origin)}" y="${y(geometry.origin) - 120}">S</text></g>${destinations}`;
    svg.dataset.routeSignature = geometry.signature;
  };
  ...
  const renderNativeNodeGlows = ({ groupNode, currentZ, stepX, stepY, scale, compact, loadedSystems }) => {
    ...
    const perfectByCoordinate = perfectSystemsByCoordinate();
    for (const coordinate of loadedCoordinates) {
      const key = `${currentZ}:${coordinate}`;
      const system = perfectByCoordinate.get(key);
      if (system) desired.set(key, system);
    }
  ...
  const renderMapMarkers = () => {
    ...
    if (!map || !content) {
      ...
      document.querySelectorAll(`.${MINI_MAP_ROUTE_CLASS}`).forEach((element) => element.remove());
      return;
    }
    ...
    syncNativeNodeGlows();
    const routeGeometry = routeMapGeometry();
    syncNativeRoutePath();
    renderMiniMapRoute(routeGeometry, currentZ);
    const markers = [];
  ...
  restoreDustCatalog(saved.dustCatalog);
  ensureUi();
  ...
  resetNativeOperationAlerts().then(() => {
    nativeOperationSchedules.clear();
    checkOperationAlerts();
  });
  renderPanel();
  renderHeaderBoosts();
  ...
  const keepNativeGlowsAttached = () => {
    ...
    if (nativeRouteNeedsSync()) syncNativeRoutePath();
  ...
  window.__stellarOdysseyIntelOverlay = {
    ...
    destroy() {
      dustDisposed = true; cancelDustRender(); dustRaster = null;
      atlasDisposed = true;
      clearTimeout(atlasPollTimer);
