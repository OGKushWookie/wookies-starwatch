(() => {
  ...
  const state = {
    ...
    sync: {
      ...
      historyPulledAt: Number(saved.sync?.historyPulledAt || saved.sync?.lastPullAt || 0),
    ...
    try {
      localStorage.setItem(STORE_KEY, JSON.stringify({
        ...
        sync: {
          ...
          historyPulledAt: state.sync.historyPulledAt,
  ...
  const applySharedSync = (payload) => {
    ...
    if (historyPulledAt > state.sync.historyPulledAt) state.sync.historyPulledAt = historyPulledAt;
    ...
    try {
      const response = await fetch(`${endpoint}/v1/sync`, {
        ...
        body: JSON.stringify({
          ...
          historySince: state.sync.historyPulledAt,
