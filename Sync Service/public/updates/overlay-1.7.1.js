(() => {
  ...
  const splitTravelSegment = (segment, context) => {
    ...
    const deltaX = Number(segment.to.x) - Number(segment.from.x);
    const deltaY = Number(segment.to.y) - Number(segment.from.y);
    const maxCoordinateDistance = maxJumpDistance / 10;
    let jumpCount = Math.max(1, Math.ceil((segment.distance - 0.000001) / maxJumpDistance));
    while (Math.hypot(Math.ceil(Math.abs(deltaX) / jumpCount), Math.ceil(Math.abs(deltaY) / jumpCount)) > maxCoordinateDistance + 0.000001) jumpCount++;
    for (let attempt = 0; attempt < 1000; attempt++, jumpCount++) {
      const points = [segment.from];
      for (let index = 1; index <= jumpCount; index++) {
        const finalPoint = index === jumpCount;
        const point = finalPoint ? segment.to : {
          name: `Waypoint ${index}`,
          kind: "waypoint",
          x: Math.round(Number(segment.from.x) + deltaX * index / jumpCount),
          y: Math.round(Number(segment.from.y) + deltaY * index / jumpCount),
          z: Number(segment.from.z ?? 1),
        };
        const previous = points.at(-1);
        if (Number(previous.x) !== Number(point.x) || Number(previous.y) !== Number(point.y) || Number(previous.z ?? 1) !== Number(point.z ?? 1)) points.push(point);
      }
      const jumps = points.slice(1).map((to, index) => {
      ...
      if (jumps.length && jumps.every((jump) => jump.legal)) return jumps;
    }
    return [{ ...segment, type: "jump", jump: 1, jumpCount: 1, legal: false }];
  };
  ...
  const renderNodes = () => {
    ...
    return `
      ...
      <section><div class="so-section-heading"><h3>Shortest route planner</h3>${state.routePlanner.destinations.length ? `<button data-action="clear-route">Clear route</button>` : ""}</div>
        <p class="so-note">Select up to ${ROUTE_MAX_DESTINATIONS} destinations. The planner finds the exact shortest visit order and compares direct flight with every currently loaded squad-portal pair. It only displays a plan—it never travels or teleports.</p>
        ...
        ${plan.order.length ? `
          ...
          `).join("")}</div>
          <p class="so-route-footnote">Every flight is split across integer system coordinates so no jump exceeds the live ${plan.context.totalFuel === null ? "unavailable" : plan.context.totalFuel.toFixed(1)}-fuel tank. Fuel uses your capped ${plan.context.fuelEfficiency.toFixed(1)}% efficiency bonus; time uses the live ${plan.context.cooldownReduction.toFixed(1)}% cooldown reduction and current engine timer. Portal costs use the departure station’s live portal level.</p>
  ...
  const ensureUi = () => {
    ...
    root.addEventListener("click", (event) => {
      ...
      if (action === "toggle") {
        ...
        state.routePlanner = { destinations: [], usePortals: true, expandedLeg: "" };
        state.operations = {};
